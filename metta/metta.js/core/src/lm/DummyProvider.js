...
const cannedResponses = {
  ...
  command: ['Command acknowledged.', 'Got it, processing your request.', 'Understood.'],
  fallback: [
...
export class DummyProvider extends BaseProvider {
  ...
  _classifyPrompt(prompt) {
    ...
    if (
      /(what|who|when|where|why|how|is|are|do|does|can|could|would|should)\b/i.test(lower) ||
      lower.includes('?')
    ) {
      return 'question';
    }
