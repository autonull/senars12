  ...
  try {
    ...
    if (spec.expectNot) {
      for (const notExpected of spec.expectNot) {
        const termStr =
          typeof notExpected.term === 'string' ? notExpected.term : notExpected.term.toString();
        const found = concepts.find((c) => c.term.toString() === termStr);
...
export function createPremise(
  ...
  return {
    term,
    type,
    truth: { f: frequency, c: confidence } as TruthType,
    label,
  };
}
...
export class ReasoningTestBuilder {
  ...
  premise(term: string, type: TaskType = 'belief', frequency = 0.9, confidence = 0.9): this {
    this.spec.premises.push({ term, type, truth: { f: frequency, c: confidence } as TruthType });
    return this;
  }
