...
export class CoreToolRegistryAdapter {
  ...
  register(spec: { name: string; description: string; parameters: Record<string, unknown>; capabilities?: ToolCapabilities; tags?: string[]; execute: (args: Record<string, unknown>, correlationId?: string, signal?: AbortSignal) => Promise<ToolResult> | ToolResult }): void {
    const tool = {
      name: spec.name,
      ...
      tags: spec.tags,
      execute: async (args: Record<string, unknown>, context?: ToolContext) => spec.execute(args, context?.chainId as any, context?.signal),
    } as any;
  ...
  get(name: string): { name: string; description: string; parameters: Record<string, unknown>; capabilities?: ToolCapabilities; tags?: string[]; execute: (args: Record<string, unknown>, correlationId?: string, signal?: AbortSignal) => Promise<ToolResult> | ToolResult } | undefined {
    ...
    return {
      ...
      tags: tool.tags,
      execute: async (args: Record<string, unknown>, correlationId?: string, signal?: AbortSignal) => tool.execute(args, { chainId: correlationId, signal } as any),
    };
  }

  list(): Array<{ name: string; description: string; parameters: Record<string, unknown>; capabilities?: ToolCapabilities; tags?: string[]; execute: (args: Record<string, unknown>, correlationId?: string, signal?: AbortSignal) => Promise<ToolResult> | ToolResult }> {
  ...
  async execute(name: string, args: Record<string, unknown>, correlationId?: string, signal?: AbortSignal): Promise<ToolResult> {
    return this.manager.execute(name, args, { chainId: correlationId, signal } as any);
  }

  getFeedback(name: string): { skill: string; lastResult: string; successRate: number; callCount: number; lastError?: string } | undefined {
  ...
  getAllFeedback(): Array<{ skill: string; lastResult: string; successRate: number; callCount: number; lastError?: string }> {
