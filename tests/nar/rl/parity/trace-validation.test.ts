...
import { TermBuilder, Truth, createTask } from '../../../../nar/src';
...
describe('RL Parity - Trace Validation', () => {
  ...
  test('causal chain: belief → value belief → goal → tool → reward → revision', async () => {
    ...
    const queryResult = nar.queryTerm(TermBuilder.inheritance(TermBuilder.atom('self'), stateTerm));
    ...
    const valueBeliefsInTrace = trace1.history.filter(t => t.term.toString() === valueTerm1.toString());
