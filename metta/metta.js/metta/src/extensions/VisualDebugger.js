...
export class VisualDebugger {
  ...
  exportDOT() {
    ...
    for (const trace of this.traces) {
      ...
      lines.push(
        `  ${fromId} -> ${toId} [label="${this._escapeDOT(trace.rule ? this._termLabel(trace.rule) : 'step')}"];`
      );
      lines.push('');
    }
    lines.push('}');
  ...
  _termLabel(term) {
    ...
    if (term.type === 'expression') {
      ...
      return `${op}(${term.components
        .slice(0, 3)
        .map((c) => this._termLabel(c))
        .join(',')}${term.components.length > 3 ? '...' : ''})`;
    }
    return '?';
  }
  ...
  hookTensorBackward(tensor) {
    ...
    if (originalBackward) {
      tensor.backward = (...args) => {
        this.recordTensorGradient(
          tensor.name || 'unknown',
          tensor.grad || tensor.data,
          tensor.layer || 'unknown'
        );
