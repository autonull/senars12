...
export class BaseComponent {
  ...
  async start() {
    return await this._executeLifecycleOperation('start', true, () => this._start(), 'startCount');
  }

  async stop() {
    return await this._executeLifecycleOperation('stop', true, () => this._stop(), 'stopCount');
  }

  async dispose() {
    if (this._disposed) {
      return this.logWarn('Component already disposed') ?? true;
    }
