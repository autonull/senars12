...
async function conductResearch(agent) {
  ...
  await runCommand(
    agent,
    'goal',
    'develop hybrid AI system combining neural and symbolic reasoning'
  );
  ...
  await runCommand(
    agent,
    'plan',
    'How to develop a hybrid AI system combining neural and symbolic reasoning'
  );
  ...
  await runCommand(
    agent,
    'reason',
    'What are the main challenges in combining neural networks with symbolic reasoning systems?'
  );
  ...
  await runCommand(
    agent,
    'think',
    'What would be the key breakthrough needed to make hybrid neural-symbolic systems practical?'
  );
  ...
  await runCommand(
    agent,
    'lm',
    'Based on the NARS knowledge and your understanding, what would be the architecture of a practical hybrid neural-symbolic system?'
  );
}
...
async function trackProgress(agent) {
  ...
  await runCommand(
    agent,
    'reason',
    'What is the current state of the hybrid AI research project and what are the next steps?'
  );
}
...
async function adaptiveReasoning(agent) {
  ...
  await runCommand(
    agent,
    'think',
    'How can we optimize symbolic reasoning speed without losing expressiveness?'
  );
  ...
  await runCommand(
    agent,
    'plan',
    'Approaches to optimize symbolic reasoning performance in hybrid systems'
  );
  ...
  await runCommand(
    agent,
    'lm',
    'Combine NARS knowledge about symbolic reasoning optimization with your expertise to propose the best approach'
  );
...
async function runCommand(agent, cmd, ...args) {
  try {
    ...
    console.log(
      `   Result: ${typeof result === 'string' ? result.substring(0, 80) : JSON.stringify(result)}${result.length > 80 ? '...' : ''}`
    );
    return result;
  } catch (error) {
    console.log(`   Command: /${cmd} ${args.join(' ')}`);
