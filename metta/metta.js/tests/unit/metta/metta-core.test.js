...
describe('Term.js - Core Data Structures', () => {
  ...
  test('List utilities work correctly', () => {
    const list = Term.exp(Term.sym(':'), [
      Term.sym('1'),
      Term.exp(Term.sym(':'), [Term.sym('2'), Term.sym('()')]),
    ]);
...
describe('TypeSystem.js - Type Inference', () => {
  ...
  test('Function type checking', () => {
    ...
    const funcType = TypeConstructors.Arrow(TypeConstructors.Number, TypeConstructors.Number);
...
describe('MeTTaInterpreter.js - Full Integration', () => {
  ...
  test('Type system integration', () => {
    ...
    const results = interpreter.query(
      Term.exp(Term.sym(':'), [Term.sym('test-func'), Term.var('type')]),
      Term.var('type')
    );
