...
describe('ReflectionOps', () => {
  ...
  test('&js-call with property access path', async () => {
    ...
    const result = await ground.execute(
      '&js-call',
      objAtom,
      grounded('nested.method'),
      grounded(21)
    );
  ...
  test('Integration: Reflection with SymbolicTensor', async () => {
    ...
    // ../../../../tensor/src/index.js
    const tensorModAtom = await ground.execute(
      '&js-import',
      grounded('../../../../tensor/src/index.js')
    );
    ...
    const shape = [2, 2];
    const tensorAtom = ground.execute(
      '&js-new',
      SymbolicTensorAtom,
      grounded(data),
      grounded(shape)
