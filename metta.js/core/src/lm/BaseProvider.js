...
export class BaseProvider extends EventEmitter {
  ...
  async process(prompt, options = {}) {
    return typeof this.generateText === 'function' ? this.generateText(prompt, options) : prompt;
  }
  ...
  async streamText(prompt, options = {}) {
    throw new Error(
      `Streaming not implemented for ${this.constructor.name}. Implement in subclass.`
    );
  }
