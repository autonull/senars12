        ...
        if (belief && belief.truth) {
          ...
          if (confidence >= this.config.minConfidenceForFeedback) {
            context.push({
              ...
              stamp: (belief as any).stamp ?? { id: 'context', creationTime: Date.now(), source: 'MEMORY' as const, derivations: [], depth: 0 },
...
Respond in Narsese format, one per line.`;
  ...
  private async parseEnrichmentResponse(response: string, term: Term): Promise<Task[]> {
