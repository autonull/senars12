...
export function generateBoundaryCases(params: {
  ...
  if (params.includeNaN) {
    cases.push({ value: Number.NaN, description: 'NaN' });
  }

  if (params.includeInfinity) {
    cases.push({ value: Number.POSITIVE_INFINITY, description: 'Infinity' });
    cases.push({ value: Number.NEGATIVE_INFINITY, description: '-Infinity' });
  }
...
export class TestTaskBuilder {
  ...
  withBudget(
    ...
    this.budget = createBudgetFn(priority ?? 0.5, durability, quality, cycles, depth);
    return this;
  }
  ...
  build(): Task {
    return createTaskFn(this.term, this.type, this.truth!, this.budget);
  }
 ...
 */
export function expectDescendingOrder<T extends { priority?: number }>(
  items: T[],
  context = 'Array'
): void {
