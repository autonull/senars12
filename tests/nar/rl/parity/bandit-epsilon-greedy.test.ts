...
import {BeliefPerceptionAdapter, GoalActionAdapter, QBeliefStore, RewardBeliefAdapter} from '../adapters/adapters';
...
describe('RL Parity - Multi-Seed Validation', () => {
    ...
    test('Level 1 parity across multiple seeds (≥10)', async () => {
        ...
        for (let seed = 0; seed < numSeeds; seed++) {
            ...
            if (diff < 0.10) { // 10% tolerance across seeds
                seedPassCount++;
            }
        }
    ...
    test('Level 2 native SeNARS across multiple seeds', {timeout: 30000}, async () => {
        ...
        for (let seed = 0; seed < numSeeds; seed++) {
            ...
            for (let ep = 0; ep < episodesPerSeed; ep++) {
                ...
                for (let step = 0; step < stepsPerEpisode; step++) {
                    ...
                    if (toolGoals.length > 0) {
                        ...
                        const exploreAction = lowConfidenceActions[Math.floor(Math.random() * lowConfidenceActions.length)];
            ...
            if (ratio > 0.5) { // Relaxed threshold for multi-seed
                seedPassCount++;
            }
        }
