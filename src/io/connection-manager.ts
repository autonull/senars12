...
export class ConnectionManager {
  ...
  constructor(logger?: Logger) {
    ...
    async addConnection(config: ConnectionConfig, deps: { nar: unknown; emit: (event: string, data: unknown) => void }): Promise<Connection> {
        ...
        const connection = factory.create(config, deps as Parameters<typeof factory.create>[1]);
    ...
    async shutdownAll(): Promise<void> {
        ...
        for (const connection of this.connections.values()) {
            if (connection.state !== 'disconnected' && connection.state !== 'idle') {
                disconnectPromises.push(connection.disconnect('shutdown').catch(err => {
                    this.logger.error(`Error disconnecting ${connection.id}`, err);
                }));
            }
        }
