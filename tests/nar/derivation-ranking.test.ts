...
describe('todo7: derivation ranking', () => {
  ...
  it('ranking tunable via CognitiveParameters + PARAMETER_SPACE', () => {
    expect(DEFAULT_COGNITIVE_PARAMETERS.inference.ranking).toEqual({
    ...
    const merged = mergeParameters({
      inference: {
        ...
        ranking: { maxAdmissions: 5, minScore: 0.5 },
      },
    });
