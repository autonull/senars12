...
export class ChannelExtension {
  ...
  async _webSearch(queryAtom) {
    ...
    try {
      ...
      if (this.agent?.toolInstances?.websearch) {
        ...
        Logger.warn(
          'Web search requested but no WebSearchTool available. Inject via toolFactories or agent.toolInstances.'
        );
        return Term.sym('()');
      }
  ...
  async _resolveFileTool() {
    ...
    Logger.warn(
      'File operation requested but no FileTool available. Inject via toolFactories or agent.toolInstances.'
    );
