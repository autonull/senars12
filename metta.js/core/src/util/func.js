...
export const PredicateFactory = Object.freeze({
  hasProperties(conditions) {
    return (item) =>
      Object.entries(conditions).every(([k, v]) =>
        typeof v === 'function' ? v(item[k]) : item[k] === v
      );
  },
  isType(type) {
  ...
  inRange(property, { min, max }) {
    return (item) => {
      ...
      return (
        typeof v === 'number' && (min === undefined || v >= min) && (max === undefined || v <= max)
      );
    };
  },
  and(...preds) {
...
import { retry, sleep, withTimeout } from './async.js';
...
export function measureTime(fn, label = 'Function') {
  return async function (...args) {
    ...
    try {
      ...
      Logger.error(
        `${label} failed after ${(performance.now() - start).toFixed(2)}ms: ${error.message}`
      );
      throw error;
    }
  };
