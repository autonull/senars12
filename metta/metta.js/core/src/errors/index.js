...
export class ParseError extends SeNARSError {
  constructor(message, { position = null, source = null, ...rest } = {}) {
    super(message, {
      code: 'PARSE_ERROR',
      details: { position, source: source?.substring(0, 100) },
      ...rest,
    });
...
export class AnalysisError extends SeNARSError {
  constructor(message, { analysisType = 'unknown', ...rest } = {}) {
    super(message, {
      code: `ANALYSIS_ERROR_${analysisType.toUpperCase()}`,
...
export class ExtensionError extends MeTTaError {
  constructor(extensionName, message, { context = {}, ...rest } = {}) {
    super(`Extension '${extensionName}' error: ${message}`, {
      context: { ...context, extensionName },
      ...rest,
    });
...
export class NeuroSymbolicError extends EnhancedError {
  ...
  static wrap(error, message, context = {}) {
    return new NeuroSymbolicError(`${message}: ${error.message}`, {
      ...context,
      originalError: error,
    });
  }
  ...
  static component(component, message, context = {}) {
    return new NeuroSymbolicError(message, {
      code: `COMPONENT_${component.toUpperCase()}`,
      ...context,
    });
  }
...
export function formatError(error) {
  return error instanceof MeTTaError
    ? error.toString()
...
export function createErrorHandler(context) {
  return (error, additionalContext = {}) => {
    ...
    throw new ReasonerError(`Error in ${context}: ${error.message}`, {
      code: 'CONTEXT_ERROR',
      originalError: error,
...
export function validateConfig(config, schema, defaults = {}) {
  ...
  for (const [key, spec] of Object.entries(schema)) {
    ...
    if (value === undefined) {
      ...
      if (!spec(value)) {
        throw new ConfigurationError(`Invalid value for ${key}`, {
          key,
          value,
          expected: spec.name,
        });
