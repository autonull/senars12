export class PerformanceTracker {
  ...
  #trackExecution(toolName, startTime, success, error) {
    ...
    this.averageExecutionTime =
      (this.averageExecutionTime * (this.successfulExecutions + this.failedExecutions - 1) +
        duration) /
      (this.successfulExecutions + this.failedExecutions);
  ...
  getStats(tools) {
    ...
    return {
      ...
      mostUsedTools: [...toolUsage.entries()]
        .sort((a, b) => b[1] - a[1])
        .slice(0, 10)
        .map(([toolName, count]) => ({
          toolName,
