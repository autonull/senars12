...
describe('todo7: proposal routing', () => {
  ...
  it('medium → sandbox-validate; high → human-approval; low in observe-only → human', () => {
    ...
    expect(
      router.route(
        gate.propose('knob-tune', { knob: 'k', value: 1 }, 'self-config-proposal'),
        'low-risk-auto-merge'
      ).route
    ).toBe('sandbox-validate');
    expect(
      router.route(
        gate.propose('patch-apply', { diff: 'x' }, 'self-patch-score'),
        'low-risk-auto-merge'
      ).route
    ).toBe('human-approval');
    expect(
      router.route(
        gate.propose('focus-weight', { focusId: 'f', weight: 0.5 }, 'self-scheduler'),
        'observe-only',
        {
          applyFocusWeight: () => {
