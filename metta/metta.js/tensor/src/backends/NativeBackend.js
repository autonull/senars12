...
export class NativeBackend extends TensorBackend {
  ...
  mul(a, b) {
    return this._elementwise(
      ...
      b,
      (x, y) => x * y,
      (g, a, b, bk) => [bk.mul(g, b), bk.mul(g, a)]
    );
  }
  ...
  div(a, b) {
    return this._elementwise(
      ...
      b,
      (x, y) => x / y,
      (g, a, b, bk) => [bk.div(g, b), bk.neg(bk.div(bk.mul(g, a), bk.mul(b, b)))]
    );
  }
  ...
  _elementwise(a, b, op, gradOp) {
    ...
    } else if (a.ndim === 2 && b.ndim === 2 && sb[0] === 1 && sa[1] === sb[1]) {
      // [m,n] + [1,n]
      data = a.data.map((v, i) => op(v, b.data[i % sa[1]]));
    } else if (a.ndim === 2 && b.ndim === 2 && sa[0] === 1 && sa[1] === sb[1]) {
      // [1,n] + [m,n]
      data = b.data.map((v, i) => op(a.data[i % sb[1]], v));
    } else if (a.ndim === 2 && b.ndim === 1 && sa[1] === sb[0]) {
      // [m,n] + [n]
      data = a.data.map((v, i) => op(v, b.data[i % sa[1]]));
    } else if (a.ndim === 1 && b.ndim === 2 && sa[0] === sb[1]) {
      // [n] + [m,n]
      data = b.data.map((v, i) => op(a.data[i % sb[1]], v));
    } else if (a.ndim === 1 && b.ndim === 2 && sa[0] === sb[0]) {
      // [n] + [n, m] (specifically [n] + [n, 1])
      // if m=1, stride is 1. i/1 = i.
      const m = sb[1];
      data = b.data.map((v, i) => op(a.data[Math.floor(i / m)], v));
    } else if (a.ndim === 2 && b.ndim === 1 && sa[0] === sb[0]) {
      // [n, m] + [n]
      const m = sa[1];
      data = a.data.map((v, i) => op(v, b.data[Math.floor(i / m)]));
    } else if (a.ndim === 2 && b.ndim === 2 && sb[1] === 1 && sa[0] === sb[0]) {
      // [m,n] + [m,1]
    ...
    } else if (a.ndim === 2 && b.ndim === 2 && sa[1] === 1 && sa[0] === sb[0]) {
      // [m,1] + [m,n]
      const n = sb[1];
  ...
  sigmoid(a) {
    return this._unaryWithGrad(
      ...
      (x) => 1 / (1 + Math.exp(-x)),
      (_, r) => {
        const oneMinus = this._createTensor(
          r.data.map((s) => 1 - s),
          r.shape
        );
  ...
  tanh(a) {
    return this._unaryWithGrad(
      a,
      (x) => Math.tanh(x),
      (_, r) => this.sub(this.ones(r.shape), this.mul(r, r))
    );
  }
  ...
  log(a) {
    return this._unaryWithGrad(a, Math.log, (i) =>
      this._createTensor(
        i.data.map((x) => 1 / x),
        i.shape
      )
  ...
  sqrt(a) {
    return this._unaryWithGrad(a, Math.sqrt, (_, r) =>
      this._createTensor(
        r.data.map((s) => 0.5 / s),
        r.shape
      )
  ...
  abs(a) {
    return this._unaryWithGrad(a, Math.abs, (i) =>
      this._createTensor(
        i.data.map((x) => (x >= 0 ? 1 : -1)),
        i.shape
      )
    );
  }
    ...
    if (axis === null) {
      ...
      if (a.requiresGrad) {
        ...
        res._gradFn = () =>
          this._accumulateGrad(
            a,
            this._createTensor(Array(a.size).fill(res.grad.data[0]), a.shape)
          );
      }
      return res;
    }
    return this._reduceAxis(
      a,
      axis,
      (v) => v.reduce((s, x) => s + x, 0),
      (a, r, ax, as, os, is, bk) => {
    ...
    if (axis === null) {
      ...
      if (a.requiresGrad) {
        ...
        res._gradFn = () =>
          this._accumulateGrad(
            a,
            this._createTensor(Array(a.size).fill(res.grad.data[0] / a.size), a.shape)
          );
      }
      return res;
    }
      ...
      if (a.requiresGrad) {
        ...
        res._gradFn = () =>
          this._accumulateGrad(
            a,
            this._createTensor(
              a.data.map((v) => (v === max ? res.grad.data[0] : 0)),
              a.shape
            )
          );
      }
      ...
      if (a.requiresGrad) {
        ...
        res._gradFn = () =>
          this._accumulateGrad(
            a,
            this._createTensor(
              a.data.map((v) => (v === min ? res.grad.data[0] : 0)),
              a.shape
            )
          );
      }
  ...
  cosineSimilarity(a, b) {
    return this.div(
      this.sum(this.mul(a, b)),
      this.mul(this.sqrt(this.sum(this.mul(a, a))), this.sqrt(this.sum(this.mul(b, b))))
    );
  }
  ...
  clamp(x, min, max) {
    ...
    const res = this._createTensor(
      x.data.map((v) => Math.max(min, Math.min(max, v))),
      [...x.shape]
    );
    if (x.requiresGrad) {
      res.requiresGrad = true;
  ...
  stack(tensors, axis = 0) {
    return this.concat(
      tensors.map((t) => this.unsqueeze(t, axis)),
      axis
    );
  }
  ...
  random(shape) {
    return this._createTensor(
      Array(shape.reduce((a, b) => a * b, 1))
        .fill(0)
        .map(Math.random),
      shape
    );
