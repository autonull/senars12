...
import type {EventBus as AgentEventBus, EventMap as AgentEventMap} from '../../../agent/src/EventBus.js';
...
export interface LMRuleConfigV2<In = unknown, Out = unknown> extends Omit<LMRuleConfig, 'promptTemplate'> {
...
export class LMRule {
    ...
    setToolDispatcher(dispatcher: (tool: string, args: Record<string, unknown>) => Promise<unknown>): void {
    ...
    async apply(primary: Term, secondary?: Term, context?: Record<string, unknown>, signal?: AbortSignal): Promise<Task[]> {
        ...
        try {
            ...
            this.emitEvent('lm.prompt', {ruleId: this.id, ruleName: this.name, prompt, timestamp: Date.now()});
