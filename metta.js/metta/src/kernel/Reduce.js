...
const getOrCreatePipeline = (interpreter, customStages = null) => {
  ...
  if (!pipeline) {
    pipeline = ReductionPipeline.createStandard(
      configManager,
      new JITCompiler(configManager.get('jitThreshold'))
    );
    pipelineRegistry.set(interpreter, pipeline);
  }
  return pipeline;
...
const getGlobalPipeline = () =>
  (globalPipeline ??= ReductionPipeline.createStandard(
    configManager,
    new JITCompiler(configManager.get('jitThreshold'))
  ));
  ...
  try {
    ...
    if (!done && value) {
      return value.deadEnd
        ? {
            reduced: { operator: atom.operator ?? atom, components: [] },
            applied: true,
            deadEnd: true,
          }
        : value;
...
export async function reduceNDAsync(
  atom,
  space,
  ground,
  limit = 10000,
