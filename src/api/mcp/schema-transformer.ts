...
import type { CapabilityDescriptor, ValidationResult } from './types.js';
...
export class SchemaTransformer {
   ...
   */
  validateArgs(args: unknown, schema: z.ZodSchema | JSONSchema7): ValidationResult {
    try {
  ...
  private isZodSchema(schema: unknown): schema is z.ZodSchema {
    return typeof schema === 'object' && schema !== null && '_def' in schema && '_def' in schema;
  }
  ...
  private isJSONSchema(schema: unknown): schema is JSONSchema7 {
    ...
    return '$schema' in obj || 'type' in obj || 'properties' in obj || 'items' in obj;
  }
}
