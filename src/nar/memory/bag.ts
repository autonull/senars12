...
const SAMPLE_FN: Record<string, (heap: BagItem<unknown>[], obj: Record<string, unknown>) => unknown> = {
    ...
    composite: (h, o) => {
        ...
        const scored = h.map(e => ({item: e.item, score: e.priority * w.priority - ((Date.now() - e.lastAccess) / 1000) * w.recency}));
...
export class Bag<T> {
  ...
  constructor(capacity: number, options?: {overflowBehavior?: OverflowBehavior; onOverflow?: (item: T, priority: number, bag: Bag<T>) => void}) {
  ...
  private getMinEntry(): BagItem<T> | undefined { return this.heap[this.heap.length - 1]; }
  ...
  add(item: T, priority: number): boolean {
    if (this._capacity === 0) { this.stats.misses++; return false; }
  ...
  serialize(): BoundedBagState<T> {
    return {
        items: this.heap.map(({item, priority, lastAccess, createdAt}) => ({item, priority, lastAccess, createdAt})),
  ...
  pruneTo(maxSize: number): void { this.heap = this.heap.slice(0, maxSize); }
  getItems(): T[] { return this.heap.map(h => h.item); }
  peek(): T | undefined { return this.heap[0]?.item; }

  remove(item: T): boolean {
    const idx = this.heap.findIndex(h => h.item === item);
    if (idx >= 0) { this.heap.splice(idx, 1); return true; }
    return false;
  }

  *entries(): Generator<[T, number]> { for (const {item, priority} of this.heap) yield [item, priority]; }
}