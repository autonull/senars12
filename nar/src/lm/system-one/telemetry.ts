...
function buildSpanAttributes(proposition: JudgmentProposition): Record<string, string | number | boolean> {
...
export function createTelemetryEmitter(sinks: TelemetrySinks = {}) {
  ...
  return function emitJudgmentResolved(proposition: JudgmentProposition, query: JudgmentQuery): void {
    try {
      if (emitEvent) {
...
export function createNarTelemetrySinks(systemEventBus: { emit: (type: string, event: any) => void }): TelemetrySinks {
  return {
