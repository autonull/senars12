...
describe('TruthTensorBridge', () => {
  ...
  describe('truthToTensor', () => {
    test('scalar mode - extracts frequency', () => {
    ...
    test('bounds mode - calculates upper and lower bounds', () => {
      const truth = { f: 0.8, c: 0.9 };
      // lower = f * c = 0.72
    ...
    test('vector mode - includes frequency, confidence, and expectation', () => {
      const truth = { f: 0.8, c: 0.9 };
    ...
    test('accepts array format [f, c]', () => {
      const tensor = bridge.truthToTensor([0.7, 0.8], 'scalar');
      expect(tensor.data[0]).toBe(0.7);
    });

    test('throws on unknown mode', () => {
      expect(() => bridge.truthToTensor({ f: 0.5, c: 0.9 }, 'unknown')).toThrow(
        'Unknown truthToTensor mode'
      );
    });
  });

  describe('tensorToTruth', () => {
    test('sigmoid mode - uses default confidence', () => {
    ...
    test('dual mode - extracts both f and c', () => {
    ...
    test('softmax mode - uses max probability', () => {
    ...
    test('throws on unknown mode', () => {
      ...
      expect(() => bridge.tensorToTruth(tensor, 'unknown')).toThrow('Unknown tensorToTruth mode');
    });
  });

  describe('round-trip conversion', () => {
    test('scalar mode preserves frequency', () => {
    ...
    test('dual mode preserves both values', () => {
      const original = { f: 0.75, c: 0.85 };
  ...
  describe('truthToExpectation', () => {
    test('calculates NAL expectation', () => {
  ...
  describe('truthsToTensor', () => {
    test('batch converts scalar mode', () => {
      const truths = [
        { f: 0.7, c: 0.8 },
        { f: 0.6, c: 0.9 },
        { f: 0.8, c: 0.7 },
    ...
    test('batch converts vector mode', () => {
      const truths = [
        { f: 0.7, c: 0.8 },
        { f: 0.6, c: 0.9 },
      ];
