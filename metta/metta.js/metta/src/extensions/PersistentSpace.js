...
export class PersistentSpace extends Space {
  ...
  _serializeAtom(atom) {
    ...
    if (!atom) {
      ...
      parts.push(0x9f, ...opData, ...compsData.flat(), 0xff);
    } else {
  ...
  _serializeVectorClocks() {
    return [...this._vectorClocks.entries()].map(([atomId, clock]) => ({
      atomId,
      clock: Array.from(clock),
    }));
  }
  ...
  setVectorClock(atom, clock) {
    this._vectorClocks.set(
      this._getAtomId(atom),
      clock instanceof Int32Array ? clock : new Int32Array(clock)
    );
  }
