...
export class SessionOrchestrator {
    ...
    setSessionInstructions(session: ConversationSession, mode: 'append' | 'replace', instructions: string): void {
        const existing = this.sessionInstructions.get(session) ?? '';
        this.sessionInstructions.set(session, mode === 'replace' ? instructions : (existing ? `${existing}\n${instructions}` : instructions));
    }
}
