...
export class APIRegistry {
  ...
  getSpec(): Record<
    string,
    { name: string; description: string; params: z.ZodSchema; returns: z.ZodSchema }
  > {
    return Object.fromEntries(
      Array.from(this.handlers.entries()).map(([name, meta]) => [
        name,
        {
          name: meta.name,
  ...
  getOpenAPISpec(): {
    ...
    return {
      ...
      paths: Array.from(this.handlers.values()).reduce(
        (acc, meta) => {
          acc[`/${meta.name}`] = {
            post: {
