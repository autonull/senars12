...
export class GridWorldEnv {
  ...
  private currentPos: GridWorldState = { row: 0, col: 0 };
  private stepCount = 0;
  ...
  step(action: number): { state: GridWorldState; reward: number; done: boolean } {
    this.stepCount++;
    const { row, col } = this.currentPos;
    let newRow = row,
      newCol = col;

    switch (action as GridAction) {
      case 0:
...
export class StochasticGridWorldEnv {
  ...
  step(action: number): { state: GridWorldState; reward: number; done: boolean } {
    ...
    let actualAction = action as GridAction;
...
export class NonStationaryBanditEnv {
  ...
  private driftMeans(): void {
    for (let i = 0; i < this.numArms; i++) {
      ...
      this.armMeans[i] = Math.max(0, Math.min(1, (this.armMeans[i] ?? 0) + change));
    }
  }
}
...
export interface MemoryPressureEnvConfig {
  baseEnv: 'bandit' | 'gridworld' | 'nonstationary';
  baseConfig: any;
  ...
  constructor(config: MemoryPressureEnvConfig) {
    ...
    switch (config.baseEnv) {
      ...
      case 'nonstationary':
        ...
        break;
      default:
        throw new Error(`Unknown baseEnv: ${config.baseEnv as string}`);
    }
  }
