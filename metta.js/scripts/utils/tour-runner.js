...
export class TourRunner {
  ...
  async runScript(command, args, options = {}) {
    ...
    if (code !== 0) {
      throw new Error(
        `Command ${command} ${args.join(' ')} failed with code ${code}. Stderr: ${stderr}`
      );
    }
  }
  ...
  async runVisualization(tour, uiPort, wsPort, outputDir, screenshotsDir) {
    if (this.movieMode) {
      ...
      await this.runEventDrivenScreenshots(
        tour,
        screenshotsDir,
        wsPort,
        `http://localhost:${uiPort}`
  ...
  async runEventDrivenScreenshots(
    tour,
    screenshotsDir,
    wsPort,
    url,
    ...
    return new Promise((resolve) => {
      ...
      ws.on('message', async (data) => {
        try {
          ...
          if (
            msg.type === 'demoState' &&
            (msg.payload.state === 'completed' || msg.payload.state === 'error')
          ) {
            clearTimeout(timer);
          ...
          if (
            !noScreenshots &&
            msg.type &&
            ['nar.cycle.step', 'reasoning.derivation'].includes(msg.type)
          ) {
            screenshotCount++;

            const p = this.runScript(
              'node',
              [
                'scripts/utils/visualize.js',
                '--type',
                ...
                path.join(
                  screenshotsDir,
                  `shot_${screenshotCount.toString().padStart(3, '0')}.png`
                ),
              ],
              { cwd: this.rootDir }
  ...
  async generateComposite(outputDir, screenshotsDir, tourId) {
    try {
      await this.runScript(
        'node',
        ['scripts/utils/generate-composite.js', '--input', screenshotsDir, '--output', outputDir],
        { cwd: this.rootDir }
      );
    } catch (e) {
