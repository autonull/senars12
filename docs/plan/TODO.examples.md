...
**Implementation**: ✅ `BaseProvider._withTimeout()` method wraps model loading with configurable timeout. Events
emitted: `lm:model-load-start`, `lm:model-load-complete`, `lm:model-load-timeout`.
...
**Note:** Full Jinja2 template support is overkill for current use cases. Most ONNX-exported models include their own
