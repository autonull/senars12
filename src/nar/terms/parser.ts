...
type TokenType = 'ATOM' | 'VARIABLE' | 'OP' | 'LPAREN' | 'RPAREN' | 'COMMA' | 'TRUTH_START' | 'TRUTH_END' | 'SEMICOLON' | 'COMMENT' | 'EOF';
...
export class TermParser {
    ...
    return statements
      ...
      .map(stmt => {
        try {
          ...
          throw new Error(`Failed to parse "${stmt}": ${error instanceof Error ? error.message : String(error)}`);
        }
      });
  }
  ...
  private tokenize(s: string): Token[] {
    ...
    let line = 1;
    let column = 1;
    const startPos = { line, column, offset: i };
    ...
    while (i < s.length) {
      ...
      if (i + 1 < s.length && s.substring(i, i + 2) === ';;') {
        i += 2;
  ...
  private parseCompoundOrAtomic(): Term {
    ...
    switch (op) {
      ...
      default:
        const pos = this.getPosition(t);
        throw new ParseError(`Unknown operator: ${op}`, pos, t);
    }
  }
