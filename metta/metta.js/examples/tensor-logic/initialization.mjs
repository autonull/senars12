...
console.log(
  'random([2,3]) U[0,1]:',
  random.toArray().map((row) => row.map((v) => v.toFixed(3)))
);
console.log(
  `  mean: ${T.mean(random).data[0].toFixed(3)}, std: ${T.std(random).data[0].toFixed(3)}`
);

const randn = T.randn([2, 3]);
console.log(
  'randn([2,3]) N(0,1):',
  randn.toArray().map((row) => row.map((v) => v.toFixed(3)))
);
...
console.log(
  '\nkaiming([3,4]) N(0, sqrt(2/3)):',
  kaiming.toArray().map((row) => row.map((v) => v.toFixed(3)))
);
