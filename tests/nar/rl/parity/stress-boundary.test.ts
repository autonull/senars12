...
import {BeliefPerceptionAdapter, GoalActionAdapter, RewardBeliefAdapter, type QBeliefStore} from '../adapters/adapters';
...
function createStressHarness(opts: {
    ...
    const perception = new BeliefPerceptionAdapter(nar, {sensorConfidence: opts.sensorConfidence ?? 0.95});
    ...
    const goalTerms = Array.from({length: NUM_ARMS}, (_, i) => actionAdapter.buildGoalTerm({name: `pull_arm_${i}`}));
...
function selectStressAction(h: StressHarness, lowConfidenceThreshold: number): number {
    ...
    const lowConfidence = h.qStore.getLowConfidenceActions(h.stateTerm, h.actions, lowConfidenceThreshold);
...
async function runBanditEpisode(h: StressHarness, steps: number, lowConfidenceThreshold = 0.4): Promise<number> {
    h.env.reset();
    let episodeReward = 0;
...
describe('RL Parity - Stress and Boundary Testing', () => {
    ...
    describe('Noise Sweep', () => {
        ...
        test('systematic sensor noise variation affects performance predictably', {timeout: 15000}, async () => {
    ...
    describe('Memory-Pressure Experiments', () => {
        ...
        test('memory pressure causes graceful degradation not catastrophic failure', {timeout: 15000}, async () => {
        ...
        test('recovery after memory pressure is removed', async () => {
            ...
            const conceptsUnderPressure = memStatsUnderPressure.pressureLevel > 0.5 ? 100 : h.nar.memory.getStatistics().totalConcepts;
