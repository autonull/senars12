...
export class Capability {
  ...
  validate() {
    ...
    if (
      [
        CapabilityTypes.PROCESS_MANAGEMENT,
        CapabilityTypes.USER_MANAGEMENT,
        CapabilityTypes.SYSTEM_CONFIGURATION,
      ].includes(this.type) &&
...
export class CapabilityManager {
  ...
  async addPolicyRule(ruleId, rule) {
    ...
    if (
      !rule.capabilities ||
      (!Array.isArray(rule.capabilities) && typeof rule.capabilities !== 'string')
    ) {
      ...
      if (toolMatch && capMatch) {
        ...
        if (rule.type === 'deny') {
          return {
            ...
            reason:
              rule.reason ||
              `Policy rule ${ruleId} denies capability ${capabilityId} for tool ${toolId}`,
          };
        }
      }
    }
