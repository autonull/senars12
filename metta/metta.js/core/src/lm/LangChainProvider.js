...
export class LangChainProvider extends BaseProvider {
  ...
  _validateConfig(config) {
    ...
    if (!['ollama', 'openai'].includes(config.provider)) {
      throw new ConfigurationError(
        `Unsupported provider type: ${config.provider}. Use 'ollama' or 'openai'.`,
        'provider'
      );
    }
  }
    ...
    try {
      ...
      if (hasTools) {
        try {
          ...
          console.warn(
            `⚠️  Model ${this.modelName} may not support tools, proceeding without tools: ${bindError.message}`
          );
      ...
      const initError =
        error instanceof InitializationError
          ...
          : new InitializationError(
              `Failed to initialize model ${this.modelName}: ${error.message}`,
              this.providerType
            );
  ...
  async generateText(prompt, options = {}) {
    ...
    return Promise.race([this._executeGenerateText(prompt), this._createTimeoutPromise(timeout)]);
  }
  ...
  _handleError(error) {
    if (error.message?.includes('model') && error.message?.includes('not found')) {
      ...
      throw new ProviderConnectionError(
        `Connection to ${this.providerType} service failed. Please ensure the service is running at ${this.baseURL}`,
        this.providerType,
        this.baseURL
      );
  ...
  async generateEmbedding() {
    throw new Error(
      "Embeddings not fully implemented for LangChainProvider due to LangChain's varied embedding support across providers"
    );
  }
}
