...
export function registerMathOps(registry) {
  const toNum = (atom) => Number.parseFloat(atom?.name) || 0;
  ...
  registry.register('&isnan-math', (x) => {
    const n = Number.parseFloat(x?.name);
  ...
  registry.register(
    '&min-atom',
    aggregateOp((...nums) => Math.min(...nums), 'EmptyOrNonNumeric')
  );
  registry.register(
    '&max-atom',
    aggregateOp((...nums) => Math.max(...nums), 'EmptyOrNonNumeric')
  );
