...
describe('Kernel Reduce', () => {
  ...
  describe('reduce - Full reduction', () => {
    test('reduces to normal form', () => {
      ...
      space.addRule(Term.exp('*', [Term.var('a'), Term.var('b')]), (bindings) => {
    ...
    test('throws on max steps exceeded', () => {
      ...
      space.addRule(Term.exp('loop', [Term.var('x')]), Term.exp('loop', [Term.var('x')]));
  ...
  describe('Integration scenarios', () => {
    ...
    test('arithmetic expression simplification', () => {
      // Rule: (+ $x 0) -> $x
      space.addRule(Term.exp('+', [Term.var('x'), Term.sym('0')]), Term.var('x'));
