...
- **Manifold-vs-LM distillation flywheel — ✅ implemented (2026-09-21).** `pnpm run demo:arcade -- --distill` closes the loop: the lm arm records its decisions into `.reports/arcade-dataset.jsonl` + vector sidecar (`LMReflex` captures the state embedding pointer at prefetch and attributes outcomes to the *decision* state — the previous perception — not the next), trains a `reflex_value` head after play (`loadTrainingData` → `trainHead` → `writeHeadArtifacts` → `.reports/arcade-heads/reflex_value`), and the manifold arm picks the head up on the next run (note: `distilled reflex_value head active`). **Verified (3 eps/game, bandit+gridworld+snake+tictactoe, Qwen3.5-9B teacher):** untrained manifold +230 → distilled +361 (student ≈ teacher's +363); bandit 360 = exactly optimal arm selection at ~0 inference cost; snake +5 vs random −450 (survives). Brier 0.40→0.07, ECE 0.32→0.14.
  - **Remaining tuning (optional):** teacher data coverage — gridworld (3-tick episodes) and tictactoe (~5-tick episodes) yield too few rows to move their heads; run more teacher episodes or distill MC-returns instead of immediate reward. Teacher exploration: the Qwen3.5-9B policy loops on snake (2-state cycle), collapsing distinct states — epsilon-injected LM proposals or multi-seed episodes would diversify. Snake LM arm could also use the placement-cascade treatment.
...
**Plan status: complete.** All D-items (D0–D24) done; Benches 35*–40 pass; `pnpm typecheck` 0 errors; `pnpm lint` clean; §8 residue (governance status surface, subpath audit, Arcade LM-arm win) also closed 2026-09-21, and the §9 "next" item (NAL as a first-class Arcade arm — rules coverage, `nal` arm with veto-fallback Negotiator, schema-induction wiring) closed same day with Bench `tests/nar/todo17b-nal-arm.test.ts` (5 tests) — full suite 204 files / 1736 tests green, `typecheck`/`lint` clean. TODO17b is done — future work belongs in a new TODO file.
...
**Next: NAL as a first-class Arcade arm — ✅ DONE (2026-09-21).** `--arms nal` (cognitive mode as a comparable arm in the summary table, forced regardless of `--mode`). Falsification Bench: `tests/nar/todo17b-nal-arm.test.ts` (5 tests — seeded trap rule ⇒ trap never executed; no rules ⇒ zero vetoes + unchanged behavior; bandit arm-0 rule; Negotiator action-match + fallback; schema-induction promotion into veto-eligible beliefs).
1. ✅ Rule coverage: honest static traps seeded where they exist — gridworld (wall bump) + bandit (arm 0 is the known-worst arm, `f=0.1,c=0.95`). snake/tetris/2048/tictactoe have **no honest static trap rule** (their `legalActions` already exclude illegal moves; traps are state-conditional) — they run rule-free and grow rules via G2 schema induction instead (`GameFocus` now constructed with `schemaInduction: true` for all cognitive arms; promotion noted per episode in the run notes).
2. ✅ nal arm added. Two Negotiator fixes made it viable: (a) `isVetoingAction` now **action-matched** — a bad-action derivation about proposal A no longer vetoes unrelated proposal B (previously it vetoed every proposal); (b) **veto-with-fallback** — the veto blocks the trap, and the best non-vetoed proposal acts instead; only when every proposal is trap-flagged does the tick yield. GameFocus books the veto even when the fallback executes (`trackVeto` after authorize). Smoke: `--arms nal` on gridworld+bandit, 2 eps → ticks=500, return +155, arm 0 vetoed every tick (never executed).
3. ✅ Schema induction already wired as above — rules grow from experience; `getPromotedSchemas()` surfaced in the arcade notes.

**Optional residue (next TODO file):** epsilon-greedy under the bandit veto keeps proposing arm 0 first every tick (equal value estimates ⇒ deterministic first-element best), so the veto fires every tick even though the fallback plays well — a reflex-side nudge (learn from `overriddenBy` to demote vetoed actions) would reduce veto churn. NAL-vs-default comparative table row beyond `harness.summary()` is unchanged (nal appears as its own arm already).

**Post-plan expansion (2026-09-21, later session) — Game collection:**
- **Env layer deleted**: `GridWorldEnv` folded into `GridWorldGame` (single plain-`Game` class; types `GridAction`/`GridWorldState`/`GridWorldConfig` re-exported from `GridWorldGame.js` — import sites updated). There is no separate `Environment` layer anywhere; README already documents `Game` as the only environment interface. GridWorld also gained honest state `terminal` (step-cap now reflected in `state()`/`observe()`) and a `render()`.
- **Game registry**: `nar/src/game/registry.ts` — `GameSpec` (name/description/actionLegend/`create(seed)`) + `GameRegistry` (register/get/create/names, `UnknownGameError` fail-loud) + `createArcadeRegistry()` (the shipped collection with demo configs; snake's action legend moved here from the arcade script). Adding a game = implement `Game` + one `GameSpec` registration; the arcade CLI takes any registered name (`--games catch,rps,arithmetic`) and skips unknown names with a note.
- **New games** (each a plain `Game`, seeded/deterministic): `catch` (falling-target paddle, ±1), `arithmetic` (a±b multiple-choice quiz — a per-step System One numeric probe), `rps` (repeated rock-paper-scissors vs a rotating deterministic opponent — structured non-stationarity). Bench: `tests/nar/game-registry.test.ts` (12 tests: registry contract, determinism per spec, per-game semantics, gridworld no-Env).
