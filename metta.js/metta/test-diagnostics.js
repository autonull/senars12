...
console.log('\n[D6] &case with expression-form branches');
{
  ...
  const vals = results
    .filter((r) => !r.toString().startsWith('=') && !r.toString().startsWith('case'))
    .map((r) => r.toString());
...
console.log('\n[D7] superpose + collapse');
{
  ...
  assertTrue(
    vals.some((v) => v.includes('a') && v.includes('b') && v.includes('c')),
...
console.log('\n[D10] map-atom with curried op');
{
  ...
  assertTrue(
    vals.some((v) => v.includes('2') && v.includes('3') && v.includes('4')),
    '  produces (2 3 4)'
  );
}
  ...
  const vals = results
    .filter(
      (r) =>
        ...
        !r.toString().startsWith('ok') &&
        !r.toString().startsWith('match-single') &&
        !r.toString().startsWith('let')
    )
    .map((r) => r.toString());
...
console.log('\n[D16] Lambda with list params');
{
  ...
  const vals = results
    .filter(
      (r) =>
        !r.toString().startsWith('=') &&
        !r.toString().startsWith('apply') &&
        !r.toString().startsWith('lambda')
    )
    .map((r) => r.toString());
