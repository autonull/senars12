...
class DemoApp {
  ...
  async setupServer() {
    ...
    try {
      ...
      console.log(
        `Discovered ${this.tools.length} tools: ${this.tools.map((t) => t.name).join(', ')}`
      );
    } catch (err) {
  ...
  async setupLLM() {
    ...
    if (choice === '1') {
      ...
      const model =
        (await ask('Enter model name (default: Xenova/Qwen1.5-0.5B-Chat): ')) ||
        'Xenova/Qwen1.5-0.5B-Chat';
      this.llm = new LocalTransformerLLM(model);
    } else if (choice === '2') {
      const model = (await ask('Enter Ollama model (default: llama3): ')) || 'llama3';
      const baseUrl =
        (await ask('Enter Base URL (default: http://localhost:11434): ')) ||
      ...
      const baseUrl = await ask(
        'Enter Base URL (e.g. http://localhost:1234/v1, leave empty for default): '
      );
      const apiKey =
        process.env.OPENAI_API_KEY ||
        (await ask("Enter API Key (default: 'not-needed'): ")) ||
        'not-needed';
  ...
  async loop() {
    while (true) {
      ...
      if (choice === '1')
        input =
          'I know that birds are animals and Tweety is a bird. based on this, is Tweety an animal? Use the reason tool.';
...
class LocalTransformerLLM {
  ...
  async chat(input, mcpClient) {
    ...
    const toolDesc = this.tools
      .map((t) => `${t.name}: ${t.description} (Schema: ${JSON.stringify(t.inputSchema)})`)
      .join('\n');
