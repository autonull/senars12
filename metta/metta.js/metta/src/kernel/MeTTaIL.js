...
export const ILLower = {
  ...
  lowerExpression(expr, ground) {
    ...
    if (op && ground?.has(op)) {
      ...
      return ILNode.groundCall(
        opId,
        comps.map((c) => this.lower(c, ground))
      );
    }

    return ILNode.expr(
      this.lower(op, ground),
      comps.map((c) => this.lower(c, ground))
    );
  },
  ...
  lowerIf(expr, ground) {
    ...
    return ILNode.if(
      this.lower(cond, ground),
      this.lower(thenB, ground),
      this.lower(elseB || ILNode.symbol('()'), ground)
    );
  },
  ...
  lowerMatch(expr, ground) {
    ...
    return ILNode.expr(ILNode.symbol('match'), [
      this.lower(target, ground),
      this.lower(pattern, ground),
      this.lower(template, ground),
    ]);
  },
};
...
export const ILOpt = {
  ...
  constantFold(il, ground) {
    ...
    if (node.kind === ILKind.GROUND_CALL && ground) {
      const allConstants = node.children.every(
        (c) => c.kind === ILKind.NUMBER || (c.kind === ILKind.SYMBOL && !isNaN(Number(c.value)))
      );
      if (allConstants) {
        try {
          const args = node.children.map((c) =>
            c.kind === ILKind.NUMBER ? c.value : Number(c.value)
          );
