export class LMStats {
  ...
  update(prompt, result, providerId, startTime, firstTokenTime = null) {
    ...
    if (firstTokenTime !== null) {
      this.avgFirstTokenLatency = this.#updateAvg(
        this.avgFirstTokenLatency,
    ...
    if (responseTime > 0 && resultTokens > 0) {
      this.avgTokensPerSecond = this.#updateAvg(
        this.avgTokensPerSecond,
        (resultTokens / responseTime) * 1000,
        this.totalCalls
      );
    }
  ...
  getCalibratedConfidence(providerId, logProb = null) {
    ...
    const confidence =
      logProb !== null ? Math.exp(logProb) * (0.5 + reputation * 0.5) : 0.6 + reputation * 0.3;
