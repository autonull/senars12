...
import {BagStrategy, Reasoner} from '../../../src/nar/reason';
import {createBudget, createTask, DEFAULT_CONFIG} from '../../../src/nar/types';
...
describe('NARExecution', () => {
    ...
    describe('run', () => {
        ...
        test('triggers rlFP.optimize() on interval', async () => {
            ...
            const execWithRLFP = new NARExecution(memory, taskManager, reasoner, configWithInterval as any, rlfp);
        ...
        test('returns total derived count', async () => {
            memory.addTask(TermBuilder.atom('test'), 'belief', Truth.TRUE, createBudget(0.9));
            taskManager.addTask(createTask(TermBuilder.atom('added'), 'belief', Truth.TRUE, createBudget(0.9)));
