/**
 * MetacognitiveMonitor - Monitoring service for cognitive operations
 *
...
export class MetacognitiveMonitor {
  ...
  detectPerformanceIssues(currentMetrics: Partial<PerformanceData>): PerformanceIssue[] {
    ...
    if (
      currentMetrics.throughput != null &&
      currentMetrics.throughput < this.config.minThroughput
    ) {
      issues.push({
        type: 'low_throughput',
        severity: 'medium',
    ...
    if (
      currentMetrics.avgProcessingTime != null &&
      currentMetrics.avgProcessingTime > this.config.maxAvgProcessingTime
    ) {
      issues.push({
    ...
    if (
      ...
      currentMetrics.memoryUsage > this.config.maxMemoryUsage
    ) {
      issues.push({
    ...
    for (const metricName of metricNames) {
      ...
      if (value !== undefined) {
        ...
        if (currentMonitor.history.length >= 2) {
          ...
          const previous = currentMonitor.history.at(-2)?.value;
