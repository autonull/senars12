  ...
  for (const event of ctx.events) {
    ...
    if (event.stage === 'act' && tool && result)
      out.push(
        result === 'ok'
          ? {
              ...base,
              type: 'tool.response',
              payload: { requestId: ctx.tickId, toolName: tool, durationMs: 0 },
            }
          : {
              ...base,
              type: 'tool.response',
              payload: { requestId: ctx.tickId, toolName: tool, error: result, durationMs: 0 },
            }
      );
      ...
      out.push({
        ...
        payload: { term: event.detail, conflictWith: 'reflex-proposal' },
      });
  }
  out.push({
    ...base,
    type: 'cycle',
    cycle: 1,
    derived: ctx.state.derivations.length,
    payload: { cycle: 1, derived: ctx.state.derivations.length },
  });
  return out;
}
