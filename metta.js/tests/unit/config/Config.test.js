...
} from '@senars/core/src/config/ConfigSchemas.js';
...
describe('Config', () => {
  describe('parse', () => {
    test('parses default arguments', () => expect(Config.parse([])).toEqual(DEFAULT_CONFIG_CORE));
...
describe('ConfigSchemas', () => {
  ...
  describe('validateConfigWithDefaults', () => {
    ...
    test('throws error for invalid config', () =>
      expect(() => validateConfigWithDefaults({ cycle: { delay: -10 } })).toThrow(
        /validation failed/i
      ));
  });
