...
export class ClosureStage extends ReductionStage {
  ...
  process(atom, context) {
    ...
    if ((!rules || rules.length === 0) && atom.operator?.operator?.operator?.name) {
      rules = context.space?.rulesFor(atom.operator.operator.operator);
    }
    if ((!rules || rules.length === 0) && atom.operator?.operator?.operator?.operator?.name) {
      rules = context.space?.rulesFor(atom.operator.operator.operator.operator);
    }
    if ((!rules || rules.length === 0) && funcAtom?.operator?.name) {
      rules = context.space?.rulesFor(funcAtom.operator);
    }
    if ((!rules || rules.length === 0) && funcAtom?.type === 'variable') {
      rules = context.space?.all() || [];
    }
