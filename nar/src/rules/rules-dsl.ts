...
import { TermBuilder, Truth, getPredicate, getSubject, termsEqual } from '../terms';
import { buildBinaryInhRule, buildInhRule, getVars } from './rule-builder.js';
import { type RuleFn, RuleRegistry, type TruthFn, createRulePattern } from './types.js';
...
const _rule = <T extends RuleDef>(
  id: string,
  description: string,
  config: Omit<T, 'id' | 'description'>
): T => ({ id, description, ...config }) as T;
...
const conversionRule = (wrap: (t: Term) => Term) =>
  buildInhRule(ID, (inh) => {
...
const buildSequenceRule = (builder: (p1: Term, p2: Term) => Term) =>
...
const deductionFromType =
  ...
  ([inh, term]: [Term, Term]): Term | undefined => {
    ...
    return termsEqual(matchOn === 'subject' ? s : p, arg)
      ? inheritance(matchOn === 'subject' ? arg : s, matchOn === 'subject' ? p : arg)
      : undefined;
  };

export const NALRules = {
  deduction: buildBinaryInhRule((l, r) => linkFn(dedExtractor)(l, r), buildDeduction),
  induction: buildBinaryInhRule((l, r) => linkFn(indExtractor)(l, r), buildInduction),
  abduction: buildBinaryInhRule((l, r) => linkFn(abdExtractor)(l, r), buildAbduction),
...
export const NALExtendedRules = {
  ...
  comparison: buildBinaryInhRule(sameInhPair, (inh1, _inh2) => {
  ...
  sameness: buildBinaryInhRule(sameInhPair, (inh1, _inh2) => {
    const { s, p } = extractInh(inh1);
    if (!s || !p) return undefined;
    return similarity(s, p);
  }),
  revisionWeak: buildBinaryInhRule(sameInhPair, (inh1, _inh2) => inh1),
  ...
  strategyEffectiveness: undefined as unknown as ([strategy, result]: [Term, Term]) =>
    | Term
    | undefined,
  resourceAllocation: undefined as unknown as ([task, resource]: [Term, Term]) => Term | undefined,
  errorPatternDetection: undefined as unknown as ([error, context]: [Term, Term]) =>
    | Term
    | undefined,
