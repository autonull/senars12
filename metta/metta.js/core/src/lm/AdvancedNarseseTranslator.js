export class AdvancedNarseseTranslator {
  ...
  toNarsese(text, options = {}) {
    ...
    for (const pattern of this.forwardPatterns) {
      ...
      if (match) {
        ...
        return this._recordTranslation(
          originalText,
          result,
          pattern.confidence,
  ...
  getQualityMetrics() {
    ...
    const totalConfidence = this.translationHistory.reduce(
      (sum, entry) => sum + entry.confidence,
      0
    );
    const highConfidence = this.translationHistory.filter(
      (entry) => entry.confidence >= this.minConfidence
    ).length;
  ...
  applyErrorCorrection(result) {
    if (
      result.narsese &&
      result.narsese.includes(' --> ') &&
      !result.narsese.endsWith('.') &&
      !result.narsese.endsWith('?') &&
      !result.narsese.endsWith('!')
    ) {
      result.narsese += '.';
