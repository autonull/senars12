...
import type { CognitiveDelta, IncomingFromServer } from '@senars/core';
...
describe('Production loop: agent deltas reach the graph', () => {
  ...
  it('emits cognitive.delta with probe terms', async () => {
    ...
    const delta = await waitFor(
      () =>
        received.find(
          (d) => d.type === 'cognitive.delta' && d.ops.some((op) => op.action === 'add_node')
        ) as (IncomingFromServer & { type: 'cognitive.delta' }) | undefined
    );
