...
(opt-in, port 8080) endpoints. Both go through the same agent and share the same per-session context.
...
`session` is optional. If provided, the bot uses that key for the session — same key across messages preserves context.
If omitted, the bot allocates per-client-id sessions automatically.
...
When `ENABLE_NL_TRANSLATION` is true (default), natural-language input is translated to Narsese via a structured LM call
before being fed to NAR. The user sees the translated Narsese as part of the response.
...
Multi-task input is supported — paragraphs are translated into multiple Narsese operations and batch-fed to NAR in a
single round.
