...
console.log(
  '\nexists([[0,1],[0,0]], axis=1):',
  b.exists(existsInput, 1).data,
  '// expected: [1, 0]'
);
...
console.log(
  'exp([1,2,3]):',
  b.exp(x).data.map((v) => v.toFixed(3))
);
console.log(
  'log([1,2,3]):',
  b.log(x).data.map((v) => v.toFixed(3))
);
console.log(
  'sqrt([1,2,3]):',
  b.sqrt(x).data.map((v) => v.toFixed(3))
);
