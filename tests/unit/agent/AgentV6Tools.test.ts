...
describe('Agent tools', () => {
    it('know stores a key/value pair', () => {
        ...
        const tools = buildAgentTools({
            know: (k, v) => { stored.push({key: k, value: v}); },
            knowGet: k => stored.find(s => s.key === k)?.value,
            knowList: () => stored,
            recall: async () => [],
        });
        const result = callTool(tools, 'know', {key: 'foo', value: 'bar'}) as {stored: boolean; key: string};
    ...
    it('know_list returns all entries', () => {
        ...
        const result = callTool(tools, 'know_list', {}) as {entries: Array<{key: string; value: string}>};
        expect(result.entries).toEqual(expect.arrayContaining([{key: 'a', value: '1'}, {key: 'b', value: '2'}]));
    });
    ...
    it('recall invokes the underlying recall function with query and limit', async () => {
        ...
        const result = await callTool(tools, 'recall', {query: 'cat', limit: 5}) as Array<{timestamp: number}>;
