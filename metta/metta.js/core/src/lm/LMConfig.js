...
export class LMConfig {
  ...
  static bindTools(provider, agent) {
    ...
    if (typeof provider.bindTools === 'function') {
  ...
  async test(name = this.active) {
    ...
    if (!config) {
      return { success: false, message: `Provider ${name} not found` };
    }
