...
export function registerArithmeticOps(registry) {
  ...
  registry.register(
    '&+',
    createReduceOp((a, b) => a + b, 0)
  );
  registry.register(
    '&*',
    createReduceOp((a, b) => a * b, 1)
  );
  registry.register(
    '&%',
    createBinaryOp((a, b) => a % b, true)
  ...
  registry.register(
    'log-math',
    mathBinary((base, x) => Math.log(x) / Math.log(base))
  );
  ...
  registry.register(
    'trunc-math',
    mathUnary((n) => Math.trunc(n), false)
  );
  ...
  registry.register('isinf-math', (...args) => {
    try {
      const [a] = OperationHelpers.requireNums(args, 1);
      return sym(!isFinite(a) && !isNaN(a) ? 'True' : 'False');
    } catch {
      return sym('False');
    }
  });
...
function createUnaryBinaryOp(unaryFn, binaryFn, checkZero = false) {
  return (...args) => {
    try {
      ...
      return OperationHelpers.error(
        exp(sym('^'), [sym('args'), ...args]),
        'Operation requires 1 or 2 args'
      );
    } catch (e) {
      return OperationHelpers.error(exp(sym('^'), [sym('args'), ...args]), e.message);
    }
