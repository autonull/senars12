...
describe('serialization adapters', () => {
  it('factorySerializable wraps PriorityBag (static deserialize) without changing its API', () => {
    ...
    const adapter = factorySerializable<
      {
        items: Array<[{ id: string; priority: number }, number]>;
        capacity: number;
        decayRate: number;
        forgetRate: number;
      },
      PriorityBag<{ id: string; priority: number }>
    >({
      serialize: () => ({
        items: [...bag.entries()],
      ...
      factory: (data) => {
        const restored = new PriorityBag<{ id: string; priority: number }>({
