...
import {createBudget, createTask, type TaskType, type Task} from '../types';
    ...
    taskGenerator: (response) => {
      ...
      return response.split('\n').filter(l => l.trim()).map(line => {
        ...
        return parsed.valid && parsed.term
          ...
          : createTask({ kind: 'atom' as const, symbol: line.trim(), hash: 0 }, 'goal', Truth.NEUTRAL, createBudget(0.8));
      });
    }
  },
  { id: 'lm-hypothesis-generation', name: 'LMHypothesisGenerationRule', description: 'Generates hypotheses from observations', priority: 0.75, promptTemplate: `${NARSESE_INSTRUCTIONS}\n\nGiven the observation "{{primaryTerm}}", what are possible explanations or hypotheses? Respond in Narsese format.`, taskGenerator: createTaskGenerator('belief', 0.6) },
  { id: 'lm-explanation-generation', name: 'LMExplanationGenerationRule', description: 'Generates explanations for beliefs', priority: 0.7, promptTemplate: `${NARSESE_INSTRUCTIONS}\n\nExplain why "{{primaryTerm}}" might be true. Provide reasoning in Narsese format.`, taskGenerator: createTaskGenerator('belief', 0.65) },
  { id: 'lm-analogical-reasoning', name: 'LMAnalogicalReasoningRule', description: 'Performs analogical reasoning between concepts', priority: 0.8, promptTemplate: `${NARSESE_INSTRUCTIONS}\n\nWhat is analogous to "{{primaryTerm}}"? Find similar patterns or structures. Respond in Narsese format.`, taskGenerator: createTaskGenerator('belief', 0.7) },
  { id: 'lm-meta-reasoning', name: 'LMMetaReasoningGuidanceRule', description: 'Provides meta-level reasoning guidance', priority: 0.75, promptTemplate: `${NARSESE_INSTRUCTIONS}\n\nAnalyze the reasoning process for "{{primaryTerm}}". What meta-level guidance can improve reasoning? Respond in Narsese format.`, taskGenerator: createTaskGenerator('belief', 0.65) },
{ id: 'lm-uncertainty-calibration', name: 'LMUncertaintyCalibrationRule', description: 'Calibrates uncertainty in beliefs', priority: 0.7, promptTemplate: `${NARSESE_INSTRUCTIONS}\n\nFor the belief "{{primaryTerm}}", what is the appropriate confidence level? Consider evidence quality. Respond in Narsese format.`, taskGenerator: createTaskGenerator('belief', 0.6) },
{ id: 'lm-schema-induction', name: 'LMSchemaInductionRule', description: 'Induces schemas from examples', priority: 0.75, promptTemplate: `${NARSESE_INSTRUCTIONS}\n\nFrom the example "{{primaryTerm}}", what general schema or pattern can be induced? Respond in Narsese format.`, taskGenerator: createTaskGenerator('belief', 0.65) },
{ id: 'lm-temporal-causal', name: 'LMTemporalCausalModelingRule', description: 'Models temporal and causal relationships', priority: 0.8, promptTemplate: `${NARSESE_INSTRUCTIONS}\n\nWhat are the temporal or causal relationships involving "{{primaryTerm}}"? Respond in Narsese format.`, taskGenerator: createTaskGenerator('belief', 0.7) },
{ id: 'lm-variable-grounding', name: 'LMVariableGroundingRule', description: 'Grounds variables in concrete instances', priority: 0.7, promptTemplate: `${NARSESE_INSTRUCTIONS}\n\nWhat concrete instances ground the variable concepts in "{{primaryTerm}}"? Respond in Narsese format.`, taskGenerator: createTaskGenerator('belief', 0.65) },
{ id: 'lm-concept-elaboration', name: 'LMConceptElaborationRule', description: 'Elaborates on concept properties', priority: 0.75, promptTemplate: `${NARSESE_INSTRUCTIONS}\n\nElaborate on the concept "{{primaryTerm}}". What are its key properties and relationships? Respond in Narsese format.`, taskGenerator: createTaskGenerator('belief', 0.7) },
{ id: 'lm-interactive-clarification', name: 'LMInteractiveClarificationRule', description: 'Seeks clarification for ambiguous inputs', priority: 0.7, promptTemplate: `${NARSESE_INSTRUCTIONS}\n\nWhat clarification is needed for "{{primaryTerm}}"? Generate questions to resolve ambiguity. Respond in Narsese format.`, taskGenerator: createTaskGenerator('question', 0.65) }
];
