...
import {createSeNARSRegistry} from '../../src/nar/lm';
import {LMService, createMockLMService} from '../../src/nar/lm';
...
const PATTERNS: Array<[RegExp, string]> = [
    [/harm|forbidden|blocked|violation|constitution/i, 'Constitution check blocked: this action is forbidden.'],
    ...
    [/trace.*rule.*bird|trace.*lm-belief/i, 'Tracing lm-belief-revision for bird concept: belief was revised with confidence 0.9.'],
    [/explain.*goal.*fly|goal.*learn.*fly/i, 'The goal of learning to fly involves acquiring flight skills for survival.'],
    ...
    [/decompose.*goal|subgoal/i, 'I will decompose this goal into subgoals: design, gather materials, build.'],
    ...
    [/current.*drive|what.*drive/i, 'Your current drives are: curiosity level 0.6, urgency level 0.3.'],
    ...
    [/what.*step|how.*build/i, 'The steps involve: gather materials, design plan, and build the structure.'],
...
export function resolveTestLMService(): LMService {
    ...
    if (provider === 'mock') {
        return createMockLMService({generateTextFn: smartMockResponse, generateObjectFn: <T>(_p: string, schema: ZodSchema<T>) => ({} as T)});
    }
