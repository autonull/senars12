...
export function registerTest(describeBlock: string, testName: string): boolean {
  ...
  if (block.testNames.has(testName)) {
    console.warn(
      `⚠️  Duplicate test name in "${describeBlock}": "${testName}"`
    );
    return false;
  }
...
export function getRegistryStats(): {
  ...
  const totalTests = Array.from(registry.values())
    .reduce((sum, block) => sum + block.testNames.size, 0);
 ...
 */
export function trackedDescribe(
  describeBlock: string,
  fn: () => void
): void {
