...
import { TermBuilder, Truth, type Term, type TruthType, type AtomicTerm } from '../../../../nar/src';
...
export class RewardBeliefAdapter {
  ...
  private readonly config: RewardBeliefAdapterConfig;
...
export interface NativeActionSelector {
  selectAction(stateId: string, nar: NAR, qStore: QBeliefStore, actionAdapter: GoalActionAdapter): number;
...
export class GridWorldSelector implements NativeActionSelector {
  ...
  selectAction(stateId: string, nar: NAR, qStore: QBeliefStore, actionAdapter: GoalActionAdapter): number {
    // Check for NAR-derived tool goals first
    const pendingGoals = nar.taskManager.getPending();
    const toolGoals = pendingGoals.filter(g => g.type === 'goal' && this.actionNames.some(n => g.term.toString().includes(`^${n}`)));
...
export class NonStationarySelector implements NativeActionSelector {
  ...
  constructor(numArms: number = 2, changeDetectionThreshold: number = 0.3, explorationRate: number = 0.2) {
    this.numArms = numArms;
  ...
  selectAction(stateId: string, nar: NAR, qStore: QBeliefStore, actionAdapter: GoalActionAdapter): number {
    ...
    if (bestAction) {
      ...
      if (bestIdx >= 0 && this.predictionErrors[bestIdx].length > 5) {
        const recentErrors = this.predictionErrors[bestIdx].slice(-5);
  ...
  onReward(stateId: string, action: number, reward: number): void {
    ...
    if (this.armPullCounts[action] > 1) {
      // Simple heuristic: if reward differs significantly from recent average
      const recentRewards = this.predictionErrors[action].slice(-10);
      if (recentRewards.length > 0) {
        const avgRecent = recentRewards.reduce((a, b) => a + b, 0) / recentRewards.length;
        const error = Math.abs(reward - avgRecent);
        this.predictionErrors[action].push(error);
        // Keep only recent errors
        if (this.predictionErrors[action].length > 20) {
          this.predictionErrors[action].shift();
        }
      }
    }
  }
...
export class NativeSenarsAgent {
  ...
  private readonly selector: NativeActionSelector;
  private readonly maxDerivationsPerStep: number;
  private readonly useTDLearning: boolean;
  private readonly gamma: number;
  private lastState: Term | null = null;
  private lastAction: Term | null = null;
  private lastStateId: string | null = null;
  ...
  async step(env: any, stateId: string): Promise<{ action: number; reward: number; done: boolean }> {
    const stateTerm = TermBuilder.atom(stateId);
  ...
  private getInitialStateId(env: any): string {
    if (env.getState) {
  ...
  private getStateId(env: any): string {
    if (env.getState) {
      const state = env.getState();
      if (typeof state === 'object' && state !== null && 'row' in state && 'col' in state) {
        return `s_${state.row}_${state.col}`;
...
export class BanditNativeAgent extends NativeSenarsAgent {
  ...
  constructor(nar: NAR, numArms: number = 3, maxDerivationsPerStep: number = 3) {
    ...
    this.numArms = numArms;
    this.actionTerms = Array.from({ length: numArms }, (_, i) => TermBuilder.atom(`^pull_arm_${i}`));
