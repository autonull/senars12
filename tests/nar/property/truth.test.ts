...
const MAX_C = Truth.MAX_CONFIDENCE;

const truthArb = fc.tuple(
fc.float({min: 0, max: 1}),
fc.float({min: 0, max: Math.fround(MAX_C), noNaN: true}).map(c => Math.min(c, MAX_C))
).map(([f, c]) => Truth.create(f, c));

describe('Truth invariants (property)', () => {
it('frequency in [0,1]', () => {
fc.assert(fc.property(fc.float(), fc.float({min: 0, max: Math.fround(MAX_C), noNaN: true}).map(c => Math.min(c, MAX_C)), (f, c) => {
...
expect(t.c).toBeLessThanOrEqual(MAX_C);
}));
});
...
expect(Math.abs(r1.f - r2.f)).toBeLessThan(0.1);
expect(Math.abs(r1.c - r2.c)).toBeLessThan(0.1);
expect(r1.c).toBeLessThanOrEqual(MAX_C);
expect(r2.c).toBeLessThanOrEqual(MAX_C);
}));
});
...
expect(r.c).toBeLessThanOrEqual(MAX_C);
}));
});
});
