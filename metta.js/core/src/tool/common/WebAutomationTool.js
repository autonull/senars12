...
export class WebAutomationTool extends BaseTool {
  ...
  async execute(params, context) {
    ...
    switch (operation.toLowerCase()) {
      ...
      default:
        throw new Error(
          `Unsupported operation: ${operation}. Supported operations: get, post, scrape, check, head`
        );
    }
  }
  ...
  validate(params) {
    ...
    if (!params.operation) {
      ...
      !['get', 'fetch', 'post', 'scrape', 'check', 'head'].includes(params.operation.toLowerCase())
    ) {
  ...
  _validateUrl(url) {
    try {
      ...
      const isAllowed = Array.from(this.allowedDomains).some(
      ...
      if (parsedUrl.protocol !== 'http:' && parsedUrl.protocol !== 'https:') {
        throw new Error(
          `Protocol "${parsedUrl.protocol}" is not allowed. Only http and https are allowed.`
        );
      }
