export class DataLoader {
  constructor(dataset, batchSize = 32, shuffle = false, collateFn = null) {
    Object.assign(this, {
      ...
      batchSize,
      shuffle,
      collateFn: collateFn ?? ((batch) => batch),
    });
  }
...
export class EarlyStopping {
  constructor(patience = 5, minDelta = 0) {
    Object.assign(this, { patience, minDelta, bestLoss: Number.POSITIVE_INFINITY, counter: 0 });
  }
...
export class MetricsTracker {
  ...
  summary() {
    return Object.fromEntries(
      Object.entries(this.history).map(([metric, values]) => {
        const latest = values[values.length - 1];
        const isMin = metric.includes('loss') || metric.includes('error');
        const best = values.reduce((acc, v) =>
          (isMin ? v.value < acc.value : v.value > acc.value) ? v : acc
        );
