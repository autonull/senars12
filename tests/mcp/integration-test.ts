...
async function runIntegrationTest() {
  ...
  try {
    ...
    console.log('Result:', (calcResult.content as Array<{ text: string }>)[0]?.text);
    ...
    console.log(
      'Beliefs count:',
      JSON.parse(((beliefsResult.content as Array<{ text: string }>)[0]?.text as string) || '[]')
        .length
    );
    ...
    console.log(
      ...
      (resourceResult.contents[0] as { text?: string } | undefined)?.text?.length ?? 0
    );
