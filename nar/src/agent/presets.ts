...
export function createAgentPreset(
  ...
  switch (preset) {
    ...
    case 'chat': {
      const nar =
        deps.nar ??
        SeNARSFactory.createDefault({ ...DEFAULT_NAR_CONFIG, lmService: deps.lmService });
    ...
    case 'lm-only': {
      ...
      const agent = createAgent({
        ...(deps.baseOptions ?? {}),
        lmService,
        ...(deps.systemInstructions
          ? { systemInstructions: deps.systemInstructions }
          : { systemInstructions: NARSESE_FALLBACK }),
      });
