...
import {ApprovalManager, createGeneralTools, createNARSTools,} from '../../../nar/src/tools/adapters';
        ...
        if (this.autonomyEngine) {
            ...
            this.reasoningHandle = setInterval(async () => {
                ...
                const urgencySteps = Math.round(
                    MIN_REASON_STEPS_PER_TICK +
                    (MAX_REASON_STEPS_PER_TICK - MIN_REASON_STEPS_PER_TICK) * urgency
                );
                const steps = Math.max(
                    MIN_REASON_STEPS_PER_TICK,
                    Math.round(urgencySteps * (this.throttle / 100))
                );
                try {
        ...
        if (this.nar) {
            ...
            Object.assign(
                ...
                createGeneralTools({
                    ...
                    episodicMemory: this.episodicMemory as Parameters<
            ...
            if (pad) {
                Object.assign(tools, {
                    ...
                    list_context: tool({
                        ...
                        inputSchema: z.object({}),
                        execute: () => ({
                            entries: [...pad.entries()].map(([k, v]) => ({key: k, value: v})),
                        }),
                    }),
                });
            }
