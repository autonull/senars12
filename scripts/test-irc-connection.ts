...
const TEST_CONFIG = {
    ...
    port: parseInt(process.env.IRC_TEST_PORT || '6667'),
...
function testIRCConnection() {
    ...
    connection.onMessage(async (message) => {
        console.log(`[IRC] Message from ${message.sender} in ${message.origin}: ${message.text.slice(0, 100)}`);
        messages.push({
...
function testMessageCreation() {
    ...
    console.log(`  Created message: ${JSON.stringify({
        id: msg.id,
        source: msg.source,
        origin: msg.origin,
        sender: msg.sender,
