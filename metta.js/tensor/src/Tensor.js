export class Tensor {
  ...
  _flatten(data) {
    ...
    return data.flat(Number.POSITIVE_INFINITY).map(Number);
  }
  ...
  transpose(axes) {
    axes ??=
      this.ndim === 2 ? [1, 0] : Array.from({ length: this.ndim }, (_, i) => this.ndim - 1 - i);
