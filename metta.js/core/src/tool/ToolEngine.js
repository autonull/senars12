...
export class ToolEngine extends BaseComponent {
  ...
  async executeTool(toolId, params = {}, context = {}) {
    ...
    const executionContext = this._createExecutionContext(
      executionId,
      toolId,
      params,
      context,
  ...
  async _validateToolCapabilities(toolId, tool) {
    const hasRequiredCapabilities = await this.capabilityManager.hasAllCapabilities(
      toolId,
      tool.capabilities ?? []
    );
    if (!hasRequiredCapabilities) {
      const missingCaps = (tool.capabilities ?? []).filter(
        (cap) => !this.capabilityManager.hasCapability(toolId, cap)
      );
  ...
  async _validateToolParams(tool, params) {
    if (tool.instance.validate && typeof tool.instance.validate === 'function') {
      ...
      if (!validationResult.isValid) {
        throw new Error(
          `Tool parameters validation failed: ${validationResult.errors?.join(', ') ?? 'Unknown error'}`
        );
      }
    }
  }
  ...
  _handleExecutionSuccess(executionContext, result, startTime, tool) {
    ...
    this.logInfo(
      `Tool execution completed: ${toolId} (${executionId}) in ${executionContext.duration}ms`
    );
  ...
  async executeTools(toolCalls, context = {}) {
    ...
    if (context.concurrent) {
      ...
      for (const call of toolCalls) {
        const result = await this.executeTool(call.toolId, call.params, {
          ...context,
          ...call.context,
        });
        results.push(result);
      ...
      if (typeof value === 'string') {
        ...
        for (const pattern of dangerousPatterns) {
          if (pattern.test(value)) {
            throw new Error(
              `Potential security risk detected in parameter${path ? ` (${path})` : ''}: ${value.substring(0, 50)}...`
            );
          }
        }
  ...
  _sanitizeResult(result) {
    ...
    if (jsonString.length > this.config.safetyLimits.maxOutputSize) {
      throw new Error(
        `Tool result exceeds maximum output size limit (${this.config.safetyLimits.maxOutputSize} chars)`
      );
    }
