...
class ScreenshotGenerator {
  ...
  async captureScreenshots(url, duration = 30000, interval = 2000, prefix = 'screenshot') {
    ...
    while (Date.now() - startTime < duration) {
      ...
      const screenshotPath = path.join(
        this.outputDir,
        `${prefix}_${count.toString().padStart(3, '0')}_${timestamp}.png`
      );
  ...
  async capturePriorityFluctuations(url, duration = 30000) {
    ...
    while (Date.now() - startTime < duration) {
      ...
      const screenshotPath = path.join(
        this.outputDir,
        `priority_fluctuation_${count.toString().padStart(3, '0')}_${timestamp}.png`
      );
  ...
  async captureDerivations(url, duration = 30000) {
    ...
    try {
      ...
      await this.page.waitForSelector('[data-testid="narsese-input-container"]', {
        timeout: 10000,
      });
    } catch (e) {
      console.log(`Navigation/Selector note: ${e.message} - continuing anyway`);
    ...
    while (Date.now() - startTime < duration) {
      ...
      const screenshotPath = path.join(
        this.outputDir,
        `derivation_${count.toString().padStart(3, '0')}_${timestamp}.png`
      );
  ...
  try {
    ...
    for (let i = 0; i < args.length; i++) {
      if (args[i] === '--type' && args[i + 1]) {
        ...
        interval = Number.parseInt(args[i + 1]);
        i++;
