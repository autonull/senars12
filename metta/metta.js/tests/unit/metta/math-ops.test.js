...
describe('Ground.js - Math Operations', () => {
  ...
  describe('Trigonometric functions', () => {
    test('sin-math calculates sine', () => {
      ...
      expect(Number.parseFloat(result.name)).toBeCloseTo(0);
    });

    test('cos-math calculates cosine', () => {
      const result = ground.execute('&cos-math', sym('0'));
      expect(Number.parseFloat(result.name)).toBeCloseTo(1);
    });
  ...
  describe('Aggregate functions', () => {
    test('min-atom finds minimum', () => {
      const list = exp(sym(':'), [
        sym('5'),
        exp(sym(':'), [sym('2'), exp(sym(':'), [sym('8'), sym('()')])]),
      ]);
    ...
    test('sum-atom calculates sum', () => {
      const list = exp(sym(':'), [
        sym('5'),
        exp(sym(':'), [sym('2'), exp(sym(':'), [sym('3'), sym('()')])]),
      ]);
