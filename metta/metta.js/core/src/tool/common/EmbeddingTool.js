...
export class EmbeddingTool extends BaseTool {
  ...
  async execute(params, context) {
    ...
    switch (operation.toLowerCase()) {
      ...
      default:
        throw new Error(
          `Unsupported operation: ${operation}. Supported operations: generate, embed, compare, similarity, cluster, search`
        );
    }
  }
  ...
  validate(params) {
    ...
    if (!params.operation) {
      ...
      !['generate', 'embed', 'compare', 'similarity', 'cluster', 'search'].includes(
        params.operation.toLowerCase()
      )
    ) {
      errors.push(
        'Invalid operation. Must be one of: generate, embed, compare, similarity, cluster, search'
      );
    }
  ...
  _validateText(text, context = 'input') {
    ...
    if (text.length > this.maxTextLength) {
      throw new Error(
        `Text ${context} exceeds maximum length limit: ${this.maxTextLength} characters`
      );
    }
  ...
  _validateTextsArray(texts, context = '', maxBatchSize = this.maxBatchSize) {
    ...
    if (texts.length > maxBatchSize) {
      throw new Error(
        `${context ? `${context} ` : ''}Batch size exceeds maximum limit: ${maxBatchSize}`
      );
    }
