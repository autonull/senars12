...
if (concept.term.toString() === termStr) {
return concept;
}
}

for (const concept of conceptMap.values()) {
if (concept.term.toString().includes(termStr)) {
return concept;
}
}
...
private getDerivationInfo(concept: Concept): unknown {
return {
status: 'derivations not tracked at concept level'
};
}

private getEvidence(concept: Concept): { supporting: unknown[]; conflicting: unknown[] } {
return {
supporting: [],
conflicting: []
};
