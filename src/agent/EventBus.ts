...
import type {EventBus as NarEventBus} from '../nar/types';
...
export interface SystemEventPayloads {
    'system:lm.rule:applied': {ruleId: string; ruleName: string; primaryTerm: string; secondaryTerm?: string; tasksProduced: number; durationMs: number; timestamp: number; schema?: string};
    'system:lm.rule:skipped': {ruleId: string; ruleName: string; reason: 'circuit_open' | 'disabled' | 'activation_failed' | 'single_premise_missing'; timestamp: number};
    'system:lm.rule:structured': {ruleId: string; schema: string; output: unknown; timestamp: number};
