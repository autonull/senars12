...
export class SymbolicTensor extends Tensor {
  constructor(data, shape, config = {}) {
    let tensorData =
      data instanceof Float32Array || data instanceof Array ? Array.from(data) : data;
  ...
  toNarseseTerm(prefix = 'tensor') {
    ...
    const symbolParts = Array.from(this.symbols).map(
      ([_, { symbol, confidence }]) => `${symbol}:${confidence.toFixed(2)}`
    );
  ...
  clone() {
    const cloned = new SymbolicTensor(new Float32Array(this.data), this.shape, {
