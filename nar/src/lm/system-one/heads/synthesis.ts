...
export function createCandidateSelectHead(options: HeadFactoryOptions): JudgmentHead {
  ...
  const calibrator = createIsotonicCalibrator(calibrationVersion, 'candidate_select');
...
export function createConflictHead(options: HeadFactoryOptions): JudgmentHead {
  const { calibrationVersion, embeddingCache, abstainThreshold } = options;
  const calibrator = createIsotonicCalibrator(calibrationVersion, 'conflict');
...
export function createGroundednessHead(options: HeadFactoryOptions): JudgmentHead {
  const { calibrationVersion, embeddingCache, abstainThreshold } = options;
  const calibrator = createIsotonicCalibrator(calibrationVersion, 'groundedness');
