...
patterns.forEach(([pattern, tensors, desc]) => {
  ...
  console.log(
    `einsum("${pattern}")`,
    Array.isArray(result.data) ? result.toArray() : result.data,
    `// ${desc}`
  );
});

const v1 = new Tensor([1, 2], { backend: T }),
  v2 = new Tensor([3, 4, 5], { backend: T });
...
console.log('\n--- Cosine Similarity ---');
const vec1 = new Tensor([1, 2, 3], { backend: T }),
  vec3 = new Tensor([-1, -2, -3], { backend: T });
