...
async function main(): Promise<void> {
    ...
    // Initial metrics
    const baselineDuration = 100; // Fixed baseline for speed comparison
    const initialMetrics = collectMetrics(rlfp, prevCoverage, baselineDuration);
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        // Collect metrics
        const metrics = collectMetrics(rlfp, prevCoverage, baselineDuration);
