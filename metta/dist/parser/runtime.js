...
function parseTokens(tokens, pos = { i: 0 }) {
    const token = tokens[pos.i];
    if (!token)
        throw new MeTTaError(ErrorCode.UNEXPECTED_TOKEN, 'Unexpected end of input');
    if (token === '(') {
        pos.i++;
        const items = [];
...
export function parseMeTTa(input) {
    const tokens = tokenize(input);
    if (tokens.length === 0)
        throw new MeTTaError(ErrorCode.UNEXPECTED_TOKEN, 'Empty input');
