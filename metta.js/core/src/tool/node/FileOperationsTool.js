...
export class FileOperationsTool extends BaseTool {
  ...
  async execute(params, context) {
    ...
    switch (operation.toLowerCase()) {
      ...
      default:
        throw new Error(
          `Unsupported operation: ${operation}. Supported operations: read, write, append, delete, list, stat, edit`
        );
    }
  }
    ...
    return {
      ...
      properties: {
        ...
        content: {
          type: 'string',
          description:
            'Content to write, append, or edit (required for write/append/edit operations)',
        },
        encoding: {
          type: 'string',
  ...
  validate(params) {
    ...
    if (!params.operation) {
      ...
      !['read', 'write', 'append', 'delete', 'list', 'stat', 'edit'].includes(
        params.operation.toLowerCase()
      )
    ) {
      errors.push(
        'Invalid operation. Must be one of: read, write, append, delete, list, stat, edit'
      );
    }
  ...
  _validateFilePath(filePath) {
    ...
    const isSafe = this.safeBaseDirs.some((safeDir) => {
      ...
      return (
        resolvedPath.startsWith(resolvedSafeDir + path.sep) || resolvedPath === resolvedSafeDir
      );
    });
