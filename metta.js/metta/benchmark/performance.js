...
async function runBenchmark() {
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    if (Number.parseInt(result[0].name) !== i + i + 1) {
  ...
  for (let i = 1; i <= 100; i++) {
    ...
    const expected = i * i;
    if (Number.parseInt(result[0].name) !== expected) {
  ...
  interpreter.space.addRule(interpreter.parser.parse('(inc 0)'), interpreter.parser.parse('1'));
  interpreter.space.addRule(
  ...
  if (Number.parseInt(result[0].name) !== 6) {
  ...
  if (Number.parseInt(complexResult[0].name) !== expectedSum) {
    console.error(
      `Complex expression failed, expected ${expectedSum}, got ${complexResult[0].name}`
    );
  }
