...
- **Implement**:
    ...
    - `analyzeTaskPatterns()` — compute real statistics from task history (distribution, success rate, complexity
      trends)

### 1.4 MetacognitiveMonitor Empty Setup

- **File**: `src/nar/self/MetacognitiveMonitor.ts:167`
- **Implement**: `setupMonitoring()` — register EventBus listeners for reasoning events, track throughput/latency/error
  rates, emit periodic performance snapshots
