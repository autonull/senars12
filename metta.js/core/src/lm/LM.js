...
export class LM extends BaseComponent {
  ...
  async generateText(prompt, options = {}, providerId = null) {
    ...
    const result = await this._executeWithCircuitBreaker(
      ProviderUtils.standardGenerate,
      provider,
      prompt,
      options
    );
  ...
  async streamText(prompt, options = {}, providerId = null) {
    ...
    return await this._executeWithCircuitBreaker(
      ProviderUtils.standardStream,
      provider,
      prompt,
      options
    );
  ...
  async process(prompt, options = {}, providerId = null) {
    ...
    if (typeof provider.process === 'function') {
      return await this._executeWithCircuitBreaker(
        provider.process.bind(provider),
        prompt,
        options
      );
    }
  ...
  _handleEmbeddingFallback(text) {
    ...
    return Array(8)
      .fill(0)
      .map((_, i) => Math.sin(textLength * (i + 1) * 0.1));
  }
