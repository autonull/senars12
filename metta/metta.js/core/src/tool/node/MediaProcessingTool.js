...
export class MediaProcessingTool extends BaseTool {
  ...
  async execute(params, context) {
    ...
    switch (operation.toLowerCase()) {
      ...
      default:
        throw new Error(
          `Unsupported operation: ${operation}. Supported operations: pdf-extract, image-ocr, text-extract, metadata, convert, image-analyze`
        );
    }
  }
  ...
  async _extractText(filePath, options = {}) {
    ...
    if (['.txt', '.md', '.csv', '.json', '.xml', '.html', '.htm'].includes(ext)) {
      ...
      if (content.length > this.maxTextLength) {
        throw new Error(
          `Text content exceeds maximum length limit (${this.maxTextLength} characters)`
        );
      }
  ...
  validate(params) {
    ...
    if (!params.operation) {
      ...
      ![
        ...
        'convert',
        'image-analyze',
      ].includes(params.operation.toLowerCase())
    ) {
      errors.push(
        'Invalid operation. Must be one of: pdf-extract, image-ocr, text-extract, metadata, convert, image-analyze'
      );
    }
  ...
  _validateFilePath(filePath) {
    ...
    if (!this.allowedFileTypes.has(ext)) {
      throw new Error(
        `File type not allowed: ${ext}. Allowed types: ${Array.from(this.allowedFileTypes).join(', ')}`
      );
    }
