...
export class MeTTaInterpreter extends BaseMeTTaComponent {
  constructor(reasoner, options = {}) {
    ...
    this.reductionCache = new ReductionCache(
      opts.cacheCapacity || configManager.get('cacheCapacity')
    );
  ...
  _extractVarAndValue(binding) {
    return binding.operator?.name === ':'
      ? binding.components
      : [binding.operator, binding.components[0]];
  }
  ...
  #processExpressions(exprs) {
    ...
    for (let i = 0; i < exprs.length; i++) {
      ...
      const isTypeAnnotation =
        (e.operator === ':' || e.operator?.name === ':') && e.components?.length === 2;
      if (isRule) {
  ...
  _processExpression(expr, results) {
    const isRule =
      (expr.operator === '=' || expr.operator?.name === '=') && expr.components?.length === 2;
    const isTypeAnnotation =
      (expr.operator === ':' || expr.operator?.name === ':') && expr.components?.length === 2;
  ...
  evaluate(atom) {
    return this.trackOperation('evaluate', () => {
      const res = reduceND(
        ...
        this.space,
        this.ground,
        configManager.get('maxReductionSteps'),
        this.reductionCache,
        this
      );
