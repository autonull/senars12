...
export function formatHumanReadable(report: TestReport): string {
    ...
    for (const scenario of report.scenarios) {
        ...
        for (const probe of scenario.probes) {
            ...
            const responsePreview = probe.response.length > 80
                ? probe.response.slice(0, 80) + '...'
                : probe.response;
    ...
    lines.push(`${'='.repeat(40)}`);
    lines.push(`Total: ${report.totals.passed} passed, ${report.totals.failed} failed (${report.totals.durationMs}ms)`);
    return lines.join('\n');
}
