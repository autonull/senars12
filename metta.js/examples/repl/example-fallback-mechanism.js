...
async function testInputRoute(agent, input) {
  ...
  try {
    ...
    console.log(
      `   → Result: ${typeof result === 'string' ? result.substring(0, 80) : JSON.stringify(result)}${result && result.length > 80 ? '...' : ''}`
    );
  } catch (error) {
...
async function testHybridReasoning(agent) {
  ...
  await agent.processInput(
    '<neural-symbolic-system --> (neural-network AND symbolic-reasoning)>. %0.85;0.80%'
  );
  ...
  await testInputRoute(
    agent,
    'Analyze the advantages of neural-symbolic systems combining both pattern recognition and logical inference'
  );
}
