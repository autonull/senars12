...
export class McpClientManager extends EventEmitter {
  ...
  async callTool(key, toolName, params = {}, retries = this.opts.maxRetries) {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        const result = await Promise.race([
          ...
          new Promise((_, rej) =>
            setTimeout(
              () => rej(new Error(`Timeout: ${toolName} (${this.opts.timeout}ms)`)),
              this.opts.timeout
            )
          ),
        ]);
    ...
    const finalError = new Error(
      `Tool ${toolName} failed after ${retries + 1} attempts: ${lastError.message}`
    );
  ...
  getStats() {
    ...
    return {
      ...
      uptime: {
        oldest:
          Math.min(
            ...Array.from(this.status.values())
              .filter((s) => s.connectedAt)
              .map((s) => now - s.connectedAt)
          ) || 0,
      },
    };
  }
