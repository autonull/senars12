...
console.log(
  `Batch size: ${batchSize}, Batches per epoch: ${Math.ceil(trainData.length / batchSize)}`
);
...
for (let epoch = 0; epoch < epochs; epoch++) {
  ...
  if (epoch % 10 === 0 || epoch === epochs - 1) {
    console.log(
      `Epoch ${epoch.toString().padStart(2)}: loss = ${(totalLoss / numBatches).toFixed(4)}`
    );
  }
}
...
paddedBatch.x.toArray().forEach((row, i) => console.log(`  Sample ${i}: [${row.join(', ')}]`));
