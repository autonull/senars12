...
export const parallelReduce = (atoms, reducer, opts = {}) => {
    ...
    return Effect.all(chunks.map((chunk) => Effect.try(() => chunk.map(reducer))), { concurrency: workers }).pipe(Effect.map((results) => results.flat()));
};
export const parallelMap = (items, mapper, opts = {}) => {
    const workers = opts.workers ?? navigator?.hardwareConcurrency ?? 4;
    return Effect.all(items.map((item) => Effect.try(() => mapper(item))), { concurrency: workers });
};
