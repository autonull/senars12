...
export class TransformersJSProvider extends BaseProvider {
  ...
  async _initialize() {
    ...
    try {
      ...
      this._emitDebug('Model loaded successfully', {
        modelName: this.modelName,
        elapsedMs: elapsed,
      });
    } catch (error) {
  ...
  async generateText(prompt, options = {}) {
    ...
    try {
      const generateOptions = {
        max_new_tokens: maxTokens ?? this.maxTokens ?? 256,
        do_sample: temp > 0,
      };
      if (temp > 0) {
      ...
      const output = await withTimeout(
        this.pipeline(inputText, generateOptions),
        this.loadTimeout,
        'Inference'
      );
      let text = Array.isArray(output)
      ...
      const stripChatML = (s) =>
        s
          .replace(/\x3c\x7c\x69\x6d\x5f\x73\x74\x61\x72\x74\x7c\x3e/g, '')
          .replace(/\x3c\x7c\x69\x6d\x5f\x65\x6e\x64\x7c\x3e/g, '');
