...
export function registerReactiveOps(interpreter) {
  ...
  const createEventAtom = (e) => {
    const dataAtom =
      e.event === 'addRule' ? exp(sym('='), [e.data.pattern, e.data.result]) : e.data;

    return exp(sym('Event'), [sym(e.event), dataAtom, sym(String(e.timestamp))]);
  };

  reg(
    '&get-event-log',
    (sinceAtom) => {
      const since = sinceAtom ? Number.parseInt(sinceAtom.name) || 0 : 0;
