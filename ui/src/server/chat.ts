...
export async function onChat(
  ...
  try {
    for await (const event of agent.chat(content, { stream: true })) {
      ...
      else if (event.kind === 'finish') send({ type: 'chat.agent.complete', content: event.text ?? '', html: renderHtml(event.text ?? ''), messageId });
      else if (event.kind === 'error') send({ type: 'chat.agent.complete', content: `Error: ${event.error}`, html: renderHtml(`Error: ${event.error}`), messageId });
    }
  } catch (e) {
    send({ type: 'chat.agent.complete', content: `Error: ${e instanceof Error ? e.message : String(e)}`, html: renderHtml(`Error: ${e instanceof Error ? e.message : String(e)}`), messageId });
  }
}
...
function replayPending(pending: { stream: string; complete: string }, send: (msg: IncomingFromServer) => void): Promise<void> {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      if (pending.complete) send({ type: 'chat.agent.complete', content: pending.complete, html: renderHtml(pending.complete), messageId: nanoid() });
      resolve();
    }, STREAM_DELAY_MS);
  });
}