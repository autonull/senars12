...
describe('TODO5b Phase 1', () => {
  ...
  it('default bindings: negotiate veto, policy deny, tool act', async () => {
    ...
    const hooks = createDefaultHooks({
      stimuli: () => [createTask(atom('cat'), 'belief', t(1, 0.9), createBudget(0.5))],
      firewall: new SymbolicFirewall(),
      memory: {
        sample: () => [
          { term: atom('cat'), priority: 0.8, beliefBag: { peek: () => ({ truth: t(1, 0.8) }) } },
        ],
      },
      ...
      policy: {
        checkCommand: (cmd) =>
          cmd === 'apply_fix' ? { allowed: true } : { allowed: false, reason: 'denylist' },
      },
      tools: {
        execute: async (name) => {
          executed.push(name);
    ...
    const vetoHooks = createDefaultHooks({
      proposers: [() => [goal]],
      negotiator: {
        resolve: () => ({ action: 'apply_fix', actionExecuted: null, vetoedBy: 'nal-test' }),
      },
      tools: {
        execute: async (name) => {
          executed.push(name);
  ...
  it('learn/validate/consolidate + core event bridge', async () => {
    ...
    const hooks = createDefaultHooks({
      ...
      validator: {
        check: (c) => ({ ok: c.state.outcomes.every((o) => o.success), reason: 'red' }),
      },
      onReward: (r) => {
        rewardSeen = r;
      },
    ...
    await runTick(
      ...
      createPipeline(
        createDefaultHooks({
          ...
          proposers: [() => [createTask(atom('y'), 'goal', t(1, 0.9), createBudget(0.9))]],
          actionOf: () => ({ name: 'do_y', args: {} }),
          negotiator: {
            resolve: () => ({ action: 'do_y', actionExecuted: 'do_y', vetoedBy: null }),
          },
    ...
    expect(
      toCognitiveEvents(red).some(
        (e) => e.type === 'tool.response' && (e.payload as { error?: string }).error === 'fail'
      )
    ).toBe(true);
  });
  ...
  it('learn uses real RLFPLearner extrinsic+intrinsic blend', async () => {
    ...
    await runTick(
      ...
      createPipeline(
        createDefaultHooks({
          tools: { execute: async () => ({ success: true }) },
          proposers: [() => [createTask(atom('z'), 'goal', t(1, 0.9), createBudget(0.9))]],
          actionOf: () => ({ name: 'do_z', args: {} }),
          negotiator: {
            resolve: () => ({ action: 'do_z', actionExecuted: 'do_z', vetoedBy: null }),
          },
          rlfp,
    ...
    const expected = rlfp.calculateRewardFromTask({
      taskType: 'meta_reasoning',
      success: true,
      metrics: { passRate: 1, contradictionReduction: 0.5 },
    });
