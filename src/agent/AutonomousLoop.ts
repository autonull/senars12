...
export class AutonomousLoop {
    ...
    private scheduleNextWake(): void {
        ...
        if (urgency > this.config.driveWakeThreshold) {
            interval = Math.max(this.config.minWakeIntervalMs, this.config.wakeIntervalMs * (1 - urgency));
        } else {
            interval = Math.min(this.config.maxWakeIntervalMs, this.config.wakeIntervalMs * (1 + urgency));
        }
    ...
    private async getRecentHistory(): Promise<string> {
        ...
        return episodes.map(e => `[${new Date(e.timestamp).toLocaleTimeString()}] ${e.type}: ${e.content}`).join('\n');
    }
...
export function createAutonomousLoop(
    ...
    return new AutonomousLoop(agent, nar, temporalMemory, contextBuilder, actionParser, reflectionEngine, wakeScheduler, driveManager, config);
}