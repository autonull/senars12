...
**Version:** 1.0 · continues TODO17.md v1.1 (Arcade plan) · lineage TODO16c.md (System One) · TODO16b.md
...
- **D23 (reconcile).** Ambiguity→Question injection: TODO16c A4 marks it shipped; this audit finds an unimplemented TODO stub at `KernelPerceptionGate.ts:237-242`. Verify against `todo16c-live-ingress.test.ts`; close whichever record is wrong.
...
- [ ] `pnpm typecheck` 0 errors, `pnpm lint` clean, full suite green
...
| DQ4 | Dead modules (CognitiveOptimizer, ObserverService, WorkingMemory, TemporalEmbeddingMemory, FeedbackLearner, src/api layer): delete or wire? | Delete, with two exceptions: port src/api's rate-limit/auth into the live servers before deletion; keep `SchemaInductor` (TODO17 G2 consumer) |
...
**Build order:** A (2d) → B (1.5d) → C (2d, DQ-gated) — each phase independently releasable; A is the only P0.
