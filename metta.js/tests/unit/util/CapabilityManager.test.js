import {
  ...
  CapabilityManager,
  CapabilityTypes,
} from '@senars/core/src/util/CapabilityManager';
...
describe('CapabilityManager', () => {
  ...
  describe('Manager', () => {
    ...
    test('granting & checking', async () => {
      ...
      await expect(
        mgr.grantCapabilities('tool1', ['c1', 'c2'], { approved: true })
    ...
    test('grant failures', async () => {
      await expect(mgr.grantCapabilities('tool1', ['missing'], { approved: true })).rejects.toThrow(
        'does not exist'
      );

      const sensitive = new Capability(CapabilityTypes.COMMAND_EXECUTION, {
    ...
    test('revocation', async () => {
      ...
      await expect(mgr.revokeCapabilities('tool1', ['c1'])).resolves.toMatchObject({
        revoked: ['c1'],
      });
    ...
    test('policy rules', async () => {
      ...
      await expect(mgr.grantCapabilities('t1', ['c1'], { approved: true })).rejects.toThrow(
        'Policy violation'
      );
    });
    ...
    test('manifests', async () => {
      ...
      expect(mgr.createSecurityManifest(m)).toMatchObject({
        id: 'm1',
        requiredCapabilities: ['c1'],
      });

      expect(() => mgr.createSecurityManifest({ ...m, requiredCapabilities: ['unknown'] })).toThrow(
        'Unknown capability'
      );
    });
  });
