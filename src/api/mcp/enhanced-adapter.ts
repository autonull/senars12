...
import type {CapabilityDescriptor, ExecutionContext, ExecutionResult, LogEntry, ProgressUpdate,} from './types.js';
...
export class EnhancedMCPAdapter extends BaseMCPAdapter {
    ...
    override getTools(): Array<{
        ...
        for (const [_name, meta] of this.registry.getHandlers()) {
            ...
            tools.push({
                ...
                inputSchema:
                    (descriptor?.inputSchema as Record<string, unknown>) || this.zodToJSONSchema(meta.params),
            });
        }
    ...
    protected async executeCapability(
        ...
        // Validate arguments
        const validation = this.schemaTransformer.validateArgs(args, handler.params);
    ...
    protected override zodToJSONSchema(schema: unknown): Record<string, unknown> {
        return this.schemaTransformer.toJSONSchema(schema as any) as Record<string, unknown>;
    }
}
