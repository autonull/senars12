...
import {
  ...
  visualizeGrid,
} from './utils/training_helpers.mjs';
...
for (let epoch = 0; epoch < 100; epoch++) {
  ...
  if (epoch % 25 === 0 || epoch === 99) {
    printMetrics(epoch, {
      ...
      acc: `${((correct / dataset.length) * 100).toFixed(1)}%`,
    });
  }
}
...
visualizeDecisionBoundary(
  ...
  15,
  [0, 7],
  dataset.map((d) => ({ x: d.x, y: d.y, label: d.label }))
);
console.log('░ = predict 0, ▓ = predict 1');

const finalCorrect = dataset.reduce(
  (acc, { x, y, label }) =>
    acc + (Math.round(model.forward(T.tensor([[x, y]])).data[0]) === label ? 1 : 0),
  0
);
