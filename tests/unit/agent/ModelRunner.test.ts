...
import type {ComposedRequest} from '../../../src/agent';
import {ModelRunner} from '../../../src/agent';
import {createMockLMService} from '../../../src/nar/lm';

function makeComposed(messages: Array<{
...
describe('ModelRunner', () => {
    it('returns empty result when no LM service', async () => {
        ...
        let result: {text: string; toolCalls: unknown[]; artifacts: unknown[]; errors: unknown[]; messages: unknown[]; usage: unknown} | undefined;
        while (true) {
