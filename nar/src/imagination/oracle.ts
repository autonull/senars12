...
export class HiddenModelOracle {
  ...
  computeOverloadKnee(
    degradationPoints: Array<{ multiplier: number; quality: number }>
  ): number | null {
...
export function createOracleFromScenario(
...
function generateHiddenRules(profile: ScenarioProfile, seed: number): HiddenRule[] {
  ...
  switch (profile) {
    ...
    case 'overload':
      return [{ term: '(data --> pattern). %0.8;0.8%', truth: Truth.create(0.8, 0.8) }];
    case 'drift':
      return [
