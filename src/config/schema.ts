...
export const narCoreSchema = z.object({
    ...
    maxDerivationsPerStep: z.number().positive().max(10000).default(narCoreDefaults.maxDerivationsPerStep),
});
...
export const botProfileSchema = z.object({
    ...
    reasoningTransparency: z.enum(['none', 'summary', 'full']).default(profileDefaults.reasoningTransparency),
});
...
export const botConfigSchema = z.object({
    reasoning: z.object({
        ...
        backgroundIntervalMs: z.number().int().positive().default(reasoningDefaults.backgroundIntervalMs),
