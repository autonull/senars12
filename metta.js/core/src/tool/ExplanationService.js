...
Summary should cover overall success/failure, key outcomes, and any important findings.`;
  ...
  _extractQuality(assessment) {
    ...
    if (
      assessment.toLowerCase().includes('high quality') ||
      assessment.toLowerCase().includes('excellent')
    ) {
      return 'high';
    } else if (
      assessment.toLowerCase().includes('low quality') ||
      assessment.toLowerCase().includes('poor')
    ) {
      return 'low';
    }
    return 'medium';
  ...
  _extractCompleteness(assessment) {
    if (
      assessment.toLowerCase().includes('complete') ||
      assessment.toLowerCase().includes('full')
    ) {
      return 'complete';
    } else if (
      assessment.toLowerCase().includes('partial') ||
      assessment.toLowerCase().includes('incomplete')
    ) {
      return 'partial';
    }
    return 'unknown';
  ...
  _extractRelevance(assessment) {
    if (
      assessment.toLowerCase().includes('highly relevant') ||
      assessment.toLowerCase().includes('very relevant')
    ) {
      return 'high';
    } else if (
      assessment.toLowerCase().includes('not relevant') ||
      assessment.toLowerCase().includes('irrelevant')
    ) {
      return 'low';
    }
    return 'medium';
  ...
  _extractScore(assessment) {
    ...
    const scoreRegex =
      /(?:score|rating):\s*(\d+(?:\.\d+)?)\s*(?:\/|out of|over)?\s*(\d+(?:\.\d+)?)/i;
    const match = assessment.match(scoreRegex);

    if (match) {
      const numerator = Number.parseFloat(match[1]);
      const denominator = Number.parseFloat(match[2]) || 10; // Assume 10 if not specified
