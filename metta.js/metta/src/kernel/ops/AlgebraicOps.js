...
export const AlgebraicOps = {
  ...
  composeMany(...spaces) {
    ...
    for (let i = 1; i < spaces.length; i++) {
      for (const atom of spaces[i].all()) {
        const functor = isExpression(atom)
          ? atom.operator?.name || atom.operator
          : atom.name || atom;
    ...
    for (const atom1 of spaces[0].all()) {
      ...
      for (let i = 1; i < spaces.length && currentMatches.length > 0; i++) {
        ...
        for (const match of currentMatches) {
          ...
          const functor = isExpression(matchConclusion)
            ? matchConclusion.operator?.name || matchConclusion.operator
            : matchConclusion.name || matchConclusion;
  ...
  tryFusion(expr) {
    ...
    for (const comp of comps) {
      if (
        isExpression(comp) &&
        (comp.operator?.name === 'compose' || comp.operator === 'compose')
      ) {
