...
export async function* processInput(
    ...
    try {
        ...
        if (nar && understandingService) {
            const nlResult = await tryNlTranslation(understandingService, generationService, nar, contextAssembler, contextOpts, input);
            if (nlResult !== null) {
                    ...
                    if (batch.meta.driveModulations) {
                        ...
                        if (driveManager) {
                            for (const [driveId, amount] of Object.entries(batch.meta.driveModulations)) {
                                driveManager.stimulate(driveId, amount);
                                yield {kind: 'drive-adjusted', text: `Adjusted ${driveId} drive.`, driveId, amount};
                            }
                        }
                    }
