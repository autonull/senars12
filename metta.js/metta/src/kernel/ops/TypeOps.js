...
export function registerTypeOps(registry) {
  ...
  registry.register(
    ...
    { lazy: true }
  ); // Prevent reduction to check actual metatype
  ...
  registry.register('&type-infer', (t, i) =>
    i?.typeChecker ? sym(i.typeChecker.typeToString(i.typeChecker.infer(t, {}))) : sym('Unknown')
  );
  registry.register('&type-check', (t, e, i) =>
    OperationHelpers.bool(i?.typeChecker?.check(t, e, {}))
  );
  registry.register('&type-unify', (t1, t2, i) =>
    sym(i?.typeChecker?.unify(t1, t2) ? 'Success' : 'Failure')
  );
}
