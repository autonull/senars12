...
export interface PromptRenderer {
     ...
     */
    render(template: PromptTemplate, args: Record<string, unknown>): MCPMessage[];
...
export class PromptManager implements PromptRenderer {
        ...
        if (template.arguments && template.arguments.length > 0) {
            const argText = template.arguments
                .map(
                    (arg) =>
                        `${arg.name}: ${
                            args[arg.name] !== undefined ? String(args[arg.name]) : '<not provided>'
                        }`
                )
                .join('\n');
     ...
     */
    createMessage(role: 'user' | 'assistant' | 'system', content: string): MCPMessage {
     ...
     */
    private validateArguments(expected: PromptArgument[], provided: Record<string, unknown>): void {
