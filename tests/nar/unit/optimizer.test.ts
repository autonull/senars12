...
describe('applyParamValues', () => {
  ...
  it('applies ranking dims preserving the sibling field', () => {
    ...
    expect(COGNITIVE_PARAMETER_SPACE.parameters['inference.rankingMaxAdmissions']).toMatchObject({
      type: 'int',
      min: 10,
      max: 1000,
    });
    expect(COGNITIVE_PARAMETER_SPACE.parameters['inference.rankingMinScore']).toMatchObject({
      type: 'float',
      min: 0,
      max: 0.5,
    });
