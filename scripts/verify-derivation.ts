...
const BINARY: Record<string, (f1: number, f2: number, c1: number, c2: number) => [number, number]> =
  {
    ...
    comparison: (f1, f2, c1, c2) => {
      const p = f1 * f2;
      return [safeDiv(p, p + (1 - f1) * (1 - f2)), c1 * c2];
    },
...
const resolveFn = (
  ruleId: string
): { kind: 'binary'; fn: BinaryFn } | { kind: 'unary'; fn: UnaryFn } | null => {
...
export function verifyRecord(
  ...
  opts: { strict?: boolean; epsilon?: number } = {}
): VerifyResult {
  ...
  for (const step of record.steps) {
    ...
    for (const premise of step.premises) {
      if (!premise || !premise.trim())
        fail('premise-shape', 'Empty premise term string', step.stepId);
    }
    if (!step.conclusion || !step.conclusion.trim())
      fail('conclusion-shape', 'Empty conclusion term string', step.stepId);

    if (step.substitution) {
      for (const [variable, value] of Object.entries(step.substitution)) {
        if (!value || !value.trim())
          fail('substitution-value', `Variable ${variable} binds empty value`, step.stepId);
        ...
        if (!inPremises)
          fail('substitution-premise', `Variable ${variable} appears in no premise`, step.stepId);
        if (!inConclusion)
          fail(
            'substitution-conclusion',
            `Neither ${variable} nor its value appears in conclusion`,
            step.stepId
          );
      }
    }
    ...
    for (const parent of step.evidenceLineage) {
      if (!knownIds.has(parent) && parent !== record.taskId) {
        fail(
          'lineage-dag',
          `Lineage ${parent} is neither a prior step nor the taskId`,
          step.stepId
        );
      }
    }
    ...
    if (!resolved) {
      ...
      if (opts.strict)
        fail(
          'unknown-rule',
          `No truth function for ruleId '${step.ruleId}' (strict mode)`,
          step.stepId
        );
      continue;
    }
    ...
    if (Math.abs(ef! - actual.f) > epsilon || Math.abs(ec! - actual.c) > epsilon) {
      fail(
        'truth-algebra',
        `Expected f=${ef!.toFixed(6)} c=${ec!.toFixed(6)}, got f=${actual.f} c=${actual.c} via ${step.ruleId}`,
        step.stepId
      );
    } else {
      truthVerified++;
...
if (isMain) {
  ...
  for (const record of records) {
    ...
    console.log(
      `${result.passed ? 'PASS' : 'FAIL'} ${result.derivationId} (truth verified: ${result.truthVerified}, skipped: ${result.truthSkipped})`
    );
