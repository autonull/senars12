...
export class ComponentManager extends BaseComponent {
  ...
  async loadComponentsFromConfig(componentConfigs) {
    ...
    if (typeof process === 'undefined' || !process.versions?.node) {
      this.logInfo(
        'ComponentManager.loadComponentsFromConfig: Skipping dynamic loading in non-Node environment'
      );
      return;
    }
  ...
  async _executeLifecycleOperation(operation, componentOrder, metricUpdate = null) {
    ...
    const verb =
      verbMap[operation] || `${operation.charAt(0).toUpperCase() + operation.slice(1)}ing`;
    ...
    for (const componentName of componentOrder) {
      ...
      this.logDebug(
        `${operation.charAt(0).toUpperCase() + operation.slice(1)}ing component: ${componentName}`
      );
  ...
  _logOperationResult(operation, successful, total, failed) {
    ...
    if (failed > 0) {
      this.logInfo(
        `${operation.charAt(0).toUpperCase() + operation.slice(1)}: ${successful}/${total} ${operation === 'init' ? 'OK' : 'successful'}, ${failed} failed`
      );
    } else {
      this.logInfo(
        `All ${total} components ${operation === 'init' ? 'OK' : `${past} successfully`}`
      );
    }
  }
  ...
  async initializeAll() {
    ...
    return await this._executeLifecycleOperation('initialize', startupOrder, {
      metric: 'initializeCount',
    });
  }
