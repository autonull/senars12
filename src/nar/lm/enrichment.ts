...
Provide a clear, concise explanation of what was derived and why.`;
  ...
  async answerQuestion(question: string, context?: Task[]): Promise<string> {
...
Respond in Narsese format, one statement per line.`;
  ...
  private parseEnrichmentResponse(response: string, term: Term): { hypotheses: Task[]; bridges: Task[] } {
        ...
        if (concept.beliefBag.size > 0) {
          ...
          if (belief) {
            relevant.push({
              ...
              stamp: (belief as any).stamp ?? { id: 'qa', creationTime: Date.now(), source: 'MEMORY' as const, derivations: [], depth: 0 },
