...
// Kernel imports
import { Term, grounded, isExpression, sym } from '../kernel/Term.js';
...
export function registerAdvancedOps(interpreter) {
  ...
  register({
    ...
    '&subst': {
      fn: (a, b, c) => {
        if (c) {
          const bindings = a.name
            ? { [a.name]: b }
            : Unify.unify(a, b) ||
              (a?.components?.length === 1 && a.operator?.name !== ':' && a.components[0].name
                ? { [a.components[0].name]: b }
                : {});
    ...
    '&let': {
      fn: (vari, val, body) => {
        ...
        const bindings =
          vari?.type === 'variable' || vari?.name?.startsWith('$')
            ? { [vari.name]: resolved }
            : Unify.unify(vari, resolved) ||
              (vari?.components?.length === 0 &&
              vari.operator?.name !== ':' &&
              (vari.operator?.type === 'variable' || vari.operator?.name?.startsWith('$'))
                ? { [vari.operator.name]: resolved }
                : null) ||
              (vari?.components?.length === 1 &&
              vari.operator?.name !== ':' &&
              (vari.components[0]?.type === 'variable' || vari.components[0]?.name?.startsWith('$'))
                ? { [vari.components[0].name]: resolved }
                : null) ||
              {};
    ...
    '&type-infer': {
      fn: (term) => {
        try {
          return sym(
            interpreter.typeChecker?.typeToString(interpreter.typeChecker.infer(term, {})) ||
              'Unknown'
          );
        } catch {
          return sym('Error');
    ...
    '&add-atom': {
      fn: (s, a) => {
        ...
        const resolveSpace = (name) =>
          name === '&self' ? interpreter.space : interpreter.spaces?.get(name) || null;
        let space, atom;
    ...
    '&length': {
      fn: (list) => {
        ...
        if (list?.operator?.name === ':' && list?.components) {
          ...
          const flattened = flattener._flattenExpr
            ? flattener._flattenExpr(list)
            : interpreter._flattenToList(list);
    ...
    '&if': {
      fn: async (cond, thenB, elseB) => {
        ...
        try {
          const results = await reduceNDAsync(
            cond,
            interpreter.space,
            interpreter.ground,
            undefined,
          ...
          if (condRes?.name === 'True') {
            const branchRes = await reduceNDAsync(
              thenB,
              interpreter.space,
              interpreter.ground,
              undefined,
    ...
    '&when': {
      fn: async (cond, body) => {
        try {
          ...
          const condRes = results[0] ?? cond;
          if (condRes?.name === 'True') {
            const bodyRes = await reduceNDAsync(
              body,
              interpreter.space,
              interpreter.ground,
              undefined,
      ...
      fn: (binds, body) => {
        ...
        for (let i = 0; i < mutablePairs.length; i++) {
          ...
          for (let j = 0; j < i; j++) {
            ...
            if (prevVari) {
              substVal = Unify.subst(
                substVal,
                { [prevVari.name]: prevResolved },
                { recursive: false }
              );
            }
          }
    ...
    '&map-fast': {
      fn: (fn, list) => {
        const flattener = interpreter.ground._flattenExpr ? interpreter.ground : interpreter;
        const listToMap = flattener._flattenExpr
          ? flattener._flattenExpr(list)
          : interpreter._flattenToList(list);
        return interpreter._listify(
          listToMap.map((el) => interpreter._reduceDeterministic(exp(fn, [el])))
        );
      },
      opts: { lazy: true },
    },
    '&filter-fast': {
      fn: (pred, list) => {
        const flattener = interpreter.ground._flattenExpr ? interpreter.ground : interpreter;
        const listToFilter = flattener._flattenExpr
          ? flattener._flattenExpr(list)
          : interpreter._flattenToList(list);
    ...
    '&foldl-fast': {
      fn: (fn, init, list) => {
        const flattener = interpreter.ground._flattenExpr ? interpreter.ground : interpreter;
        const listToFold = flattener._flattenExpr
          ? flattener._flattenExpr(list)
          : interpreter._flattenToList(list);

        return listToFold.reduce(
          (acc, el) => interpreter._reduceDeterministic(exp(fn, [acc, el])),
          init
        );
      },
      opts: { lazy: true },
    ...
    'reduce-atom': {
      fn: (list, accVar, elVar, op) => {
        const flattener = interpreter.ground._flattenExpr ? interpreter.ground : interpreter;
        const elements = flattener._flattenExpr
          ? flattener._flattenExpr(list)
          : interpreter._flattenToList(list);
