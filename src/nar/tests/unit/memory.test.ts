...
describe('Memory', () => {
    ...
    describe('addTask', () => {
        test('adds belief task', () => {
            const term = TermBuilder.atom('bird');
            const result = mem.addTask(term, 'belief', Truth.TRUE, 0.8);
        ...
        test('adds goal task', () => {
            ...
            const result = mem.addTask(term, 'goal', undefined, 0.9);
            expect(result).toBe(true);
    ...
    describe('consolidate', () => {
        test('decays and removes low priority after interval', () => {
            const term = TermBuilder.atom('bird');
            mem.addTask(term, 'belief', Truth.TRUE, 0.1);
        ...
        test('keeps high priority concepts', () => {
            const term = TermBuilder.atom('bird');
            mem.addTask(term, 'belief', Truth.TRUE, 0.9);
    ...
    describe('forgetting', () => {
        test('removes oldest when at capacity', () => {
            ...
            smallMem.addTask(TermBuilder.atom('a'), 'belief', Truth.TRUE, 0.5);
            smallMem.addTask(TermBuilder.atom('b'), 'belief', Truth.TRUE, 0.5);
    ...
    describe('sample', () => {
        test('returns concepts sorted by priority', () => {
            ...
            mem.addTask(a, 'belief', Truth.TRUE, 0.5);
            mem.addTask(b, 'belief', Truth.TRUE, 0.5);
            mem.addTask(b, 'belief', Truth.TRUE, 0.5);
