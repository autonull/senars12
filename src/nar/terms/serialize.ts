...
const NARY_OPS_SET = new Set(Object.entries(OPERATORS).filter(([, v]) => v.nary).map(([k]) => k));
...
const serialize = (term: Term): string => {
    ...
    const serializeArgs = (args: readonly Term[]): string => args.map((a: Term) => serialize(a)).join(', ');

    if (NARY_OPS_SET.has(term.kind)) {
        const args = term.args ?? ([] as readonly Term[]);
        const sep = NARY_SEPARATORS[term.kind] ?? ', ';
        if (args.length === 0) return term.kind === 'conjunction' ? 'TRUE' : term.kind === 'disjunction' ? 'FALSE' : '';
