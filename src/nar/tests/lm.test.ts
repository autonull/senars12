import {LMResponseParser, MockLMClient, RuleBasedLMClient} from '../lm';
...
describe('LMResponseParser', () => {
    describe('parse', () => {
        ...
        test('extracts term from text with surrounding content', () => {
            const result = LMResponseParser.parse('Based on analysis, (bird --> animal) is a key relationship');
            expect(result.valid).toBe(true);
        });
        ...
        test('handles malformed JSON gracefully', () => {
            const result = LMResponseParser.parse('{"narsese": "(A --> B)", truth: broken');
            expect(result.valid).toBe(true);
        });
    });
