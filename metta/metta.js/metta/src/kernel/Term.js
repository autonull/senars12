...
export const exp = (operator, components) => {
  ...
  for (let i = 0; i < components.length; i++) {
    if (!components[i]) {
      throw new Error(
        `Invalid component at index ${i} in expression: ${operator.toString?.() || operator}`
      );
    }
  }
...
export const isVariable = (a) =>
  a?._typeTag === TYPE_VARIABLE || (a?.type === 'atom' && isVariableName(a.name));
export const isSymbol = (a) =>
  a?._typeTag === TYPE_SYMBOL || (a?.type === 'atom' && !a.operator && !isVariableName(a.name));
...
export const isList = (a) =>
  isExpression(a) && a.operator?.name === ':' && a.components?.length === 2;
