...
| `SourceKey` | type | Finer reputation keys: `provider:<name>`, `domain:<host>` (both landed, Phase E); `peer:<id>` **future** — peer delegation never landed, no reader/writer exists. |
...
### Phase E — strategy composition + consensus/reputation ✓ (Bench 90, `tests/nar/refactor2-strategy-consensus.test.ts`, 24→26 tests)
...
- `MettaProposer` at `nar/src/reflex/metta-proposer.ts`: sync evaluator seam (`(expr) => boolean | null`) — nar has no `@senars/metta` dep; canonical wiring is `createMeTTa()` + `Effect.runSync(engine.evaluate(parseMeTTa(expr)))` (verified working). Confidence-1.0 votes on engine agreement; abstains on False/null (never vetoes). **Phase F (audit M1)**: `toExpression` is now **required** — the old optional default emitted `(= (eval <action>) True)`, but the MeTTa stdlib has no `eval` op, so a zero-config proposer abstained forever (see §10a). ⚠️ MeTTa `=` is **structural-shallow** (args not pre-reduced: `(= (+ 2 2) 4)` → False; only ground pairs like `(= 4 4)` evaluate True) — proposers must compare pre-reduced forms. `WeightedQuorum`/`MettaProposer` exported from `@senars/nar/reflex`.
...
---
**Improvement opportunities**: consolidated in §10 (items 1–5 are the A–D list; no duplicates kept here).
...
## 10a. Post-completion audit — gaps found reviewing the landed work (2026-09-25)

Findings from re-auditing TODO1+TODO2 with fresh eyes; ordered by significance. Items marked **→ M1..M4** are planned as Phase F (below).

1. **[→ M1] `MettaProposer` default `toExpression` can never vote.** Default emits `(= (eval <action>) True)`, but the MeTTa stdlib has **no `eval` op** (metta/src/stdlib/index.ts:37-166 — arithmetic, comparison, list, string, control-flow ops only). Verified: the default expression reduces to `False` for every action ⇒ the class as constructed with zero options abstains forever. Bench 90 missed it because every test injected a custom `toExpression`. Fix: default to a *predicate-injection* contract (caller supplies per-action facts via `toExpression`; make the parameter required, or ship a working trivial default) + a bench asserting the zero-option constructor actually votes when the engine agrees.

2. **[→ M2] Fine reputation keys have readers but no writers.** Phase E added `provider:<name>`/`domain:<host>` *consumption* (ingress judge, perception gate), but the only `record()` producers still use legacy keys: `llm-narration` (bot groundedness gate — should be `provider:<activeLM>`), `user` (`.react`), `system-one` (judge fallback). So a flaky LM provider is never marked down at the key the judge actually reads. Fix: bot's groundedness gate records `providerKey(lmService.provider)`; keep legacy-key writes alongside during transition (append-compatible JSONL, no migration).

3. **[→ M3] `curriculum.ts` `sourceReputation` feed is dead in production.** The bot `.probes` wiring passes `multiplier: (key) => nar.getSourceReputation?...` — but Phase E's reaction episodes all carry `sourceKey: 'user'` (the only writer), so every correction shares one key and the boost is uniform ⇒ no selection effect. Meaningful use needs per-channel `sourceKey`s at capture (e.g. provenance of the *turn being corrected*: LM narration turns → `provider:<name>`, peer turns → `peer:<id>`). Pairs with M2.

4. **[→ M4] TS18030 private-name `?.` chains in bot.ts** (`cortex?.#defaultGrammar` etc., 4 sites) — invalid TS that the repo never sees (`src/bin/**` excluded from tsconfig; only a scoped manual check catches it). Rewrite to non-chained access (`('#defaultGrammar' in cortex ? …)` style or accessor getters). Same exclusion already hid the `wired.*` scope bug (fixed in Phase E).

5. **[→ M5] `NAREventMap`/SelfMetaGame contradiction intake** (the plan's unrealized "MeTTa/NAL disagreement ⇒ Contradiction event"): still zero typed event + zero intake; SelfMetaGame's only contradiction signal remains the `nar-execution.ts` substring sniff. Keep deferred, but fold into M1's consensus work if a real disagreement source appears.

6. **[→ M6] `deprecated` `createPipeline` alias** still re-exported (`nar/src/tick/index.ts:36`) and still the *imported* symbol in `nar-execution.ts:12` — the alias outlived its 2-minor keep window and even the internal code prefers it. Sweep: import `createTickPipeline` internally, drop the alias re-export next major.

7. **[→ M7] §8 Progress duplicates §10 items 1–5 verbatim** ("New improvement opportunities (from A–D)" vs "From A–D" lists) — same text twice in one file. Consolidate into §10 only.

8. **[→ M8] plan drift hygiene**: Phase E progress section sits *above* Phase D (out of order), and `peer:<id>` keys (naming table §3) have neither reader nor writer — document as not-yet-existing (peer delegation never landed) rather than implying parity with provider/domain keys.

### Phase F — audit remediations ✓ (2026-09-25; same session, Bench 90 grown to 26 tests)

- **M1 ✓**: `MettaProposer.toExpression` is now **required** (no default — the stdlib has no `eval` op, so the old default abstained forever). Bench 90 gained an explicit test: fact-table source votes on grounded actions, skips unknowns.
- **M2 ✓**: bot groundedness gate records `provider:<name>` (via `providerKey(resolveLMSettings().provider)`, resolved live) **alongside** the legacy `llm-narration` key.
- **M3 ✓**: `DialogueCaptureDeps.sourceKey?: () => string` — dialogue + reaction episodes stamp the live provider key (default `'user'`); bot wires `providerKey(resolveLMSettings().provider)`. The curriculum feed now has a real join: Bench 90 asserts a degraded provider's corrections rank first.
- **M4 ✓**: `SystemOneDispatcher.describe()` + `LMServiceCortex.describe()`/`setRuntimeTuning()` (interface `describe?` optional, StubCortex-compatible via `?.`); manifold `getHeads()`. All private-name `?.` chains and `#field` writes in bot.ts replaced (`cortex model|grammar` runtime-set now goes through `setRuntimeTuning` — the old `#field` assignments were invalid TS and would have thrown). Also fixed en route: `episodicMemory`→`episodic` kwarg in the bot MemoryQuery construction, private `proposalRouter` reach-in in `.gates proposals` (now `getGovernanceQueues()`), and an `instanceof (await import(...)).then(...)` brokenness in `.cortex`. **`tsconfig.bin.json` + `pnpm typecheck:bin` added** (src/bin + src/cli): bot.ts went 65 errors → 18; the remainder is pre-existing strictness debt, now visible and greppable (§9).
- **M6 ✓**: `createPipeline` alias removed from the `nar/src/tick` barrel re-export (deprecated alias outlived its window; the tick.ts definition + `@deprecated` JSDoc stay for the remaining import path); the one barrel consumer (todo5b-otel test) imports `createTickPipeline` with a local alias. The `nar-execution.ts` `createPipeline` import is the **stream** module (different function, not deprecated).
- **M7+M8 ✓**: §8 opportunities block deduplicated into §10; progress sections reordered A→E; `peer:<id>` annotated future in §3.
- Deferred (unchanged): #5 contradiction intake (no producer), #6 expression emission (needs per-strategy latency data), #11 learn() (no use case), §9 pre-existing items (monolith budget, memory-query flake, deps-gate baseline, remaining bot.ts strictness debt).
