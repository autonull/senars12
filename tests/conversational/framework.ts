...
export interface HarnessOptions {
  lmService?: LMService;
  agentOptions?: Partial<CreateAgentConfig>;
  verbose?: boolean;
  timeoutMs?: number;
}
...
export class ConversationalTestHarness {
  private agent: Awaited<ReturnType<typeof createAgent>> | undefined;
  private nar: NAR | undefined;
  private readonly lmService: LMService | undefined;
  private readonly agentOptions: Partial<CreateAgentConfig>;
  ...
  getAgent(): Awaited<ReturnType<typeof createAgent>> | undefined {
    return this.agent;
  }
  ...
  async probe(input: string, expectations?: ProbeExpectations): Promise<ProbeResult> {
    ...
    try {
      ...
      for await (const ev of stream) {
        if (ev.kind === 'text-delta' && ev.text) response += ev.text;
