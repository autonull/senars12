...
export const sum = (values) =>
  Array.isArray(values) ? values.reduce((acc, val) => acc + val, 0) : 0;
export const min = (values) =>
  Array.isArray(values) && values.length > 0 ? Math.min(...values) : Number.POSITIVE_INFINITY;
export const max = (values) =>
  Array.isArray(values) && values.length > 0 ? Math.max(...values) : Number.NEGATIVE_INFINITY;
