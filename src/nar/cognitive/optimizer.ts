...
import type {MetricsSummary, SearchSpace} from '../strategies/types.js';
...
export const COGNITIVE_PARAMETER_SPACE: SearchSpace = {
    parameters: {
        ...
        'strategy.premise': {
            type: 'categorical',
            values: ['default-formation', 'bag', 'prolog', 'resolution', 'goal-driven', 'analogical', 'term-link', 'task-match', 'decomposition', 'exhaustive']
        },
        ...
        for (const [key, param] of entries) {
            ...
            switch (param.type) {
                ...
                case 'float': {
                    ...
                    const mid = (lo + hi) / 2;
                    const grid = param.log
                        ? [Math.exp(lo), Math.exp(mid), Math.exp(hi)]
                        : [lo, mid, hi];
                    grids.push(grid);
                    break;
                }
...
export class CognitiveOptimizer {
    ...
    async optimize(algorithm: 'grid' | 'random', budget: {
