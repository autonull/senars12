...
async function runAdapterWrapped(
  env: any,
  baseline: any,
  episodes: number,
  steps: number,
...
async function runNativeSenars(
  env: any,
  episodes: number,
  steps: number,
  envType: EnvType,
...
async function runExperiment(
  seed: number
): Promise<{ baselineRewards: number[]; senarsRewards: number[] }> {
  let env: any;
  let baseline: any;
  ...
  if (mode === 'direct') {
    ...
    baselineRewards = await runDirectBaseline(env, baseline, episodesPerSeed, stepsPerEpisode);
    senarsRewards = await runAdapterWrapped(
      env,
      baseline,
      episodesPerSeed,
      stepsPerEpisode,
...
async function main() {
  ...
  for (let seed = 0; seed < numSeeds; seed++) {
    ...
    console.log(
      `  Baseline: ${metrics.avgBaselineReward.toFixed(3)} ± ${metrics.baselineStd.toFixed(3)}`
    );
    console.log(
      `  SeNARS:   ${metrics.avgSenarsReward.toFixed(3)} ± ${metrics.senarsStd.toFixed(3)}`
    );
  ...
  const csvRows = allResults
    .map(
      (r) =>
        `${r.seed},${r.avgBaselineReward.toFixed(4)},${r.avgSenarsReward.toFixed(4)},${r.ratio.toFixed(4)}`
    )
    .join('\n');
