...
| 76 | DQ2 heuristic reaction attribution | `tests/nar/todo24-heuristic-attribution.test.ts` | ✅ **green** |
...
- DQ2 close-out pass (2026-09-25, bench-first): heuristic reaction attribution behind `dialogue.attribution: 'cues'` (default `'explicit'`). Files: `nar/src/dialogue/attribution.ts` (NEW), `nar/src/dialogue/{capture,index}.ts` (wired + exported), `util/src/config/dialogue.ts` (`attribution` knob), `tests/nar/todo24-heuristic-attribution.test.ts` (NEW, Bench 76). Attribution is adjacency-only (the utterance *following* a turn reacts to it), never synthesizes turns, never double-binds. All design decision points (DQ1–DQ7) are now closed.
...
7. ~~**Narsese-level correction formalization** (DQ6)~~ — ✅ **done** (bench-first, per the gate note: **Bench 75** `tests/nar/todo24-narsese-corrections.test.ts` falsifies. `DialogueCaptureDeps.formalize` maps correction text (available only at bind time, I6) → formalization candidates; `bindReaction('correct', …)` stores bounded `Lesson`s (`source: 'reaction'`, truth = formalization confidence floored at 0.5, provenance = the corrected turnId) exposed via `capture.lessons`. `.lessons` merges them with retrospective lessons under the same explicit `nar.input` ingestion — nothing auto-applies (I2/I3). Cost gate: shares the `dialogue.captureAll` gate with the enrich formalizer, so no LM call unless full-fidelity capture is opted in. **Heuristic reaction attribution** (DQ2) — ✅ **done** (bench-first: **Bench 76** `tests/nar/todo24-heuristic-attribution.test.ts` gates it on scripted ground truth: precision ≥ 0.95 over bound pairs, **zero** false positives on neutral continuations, default `dialogue.attribution: 'explicit'` never fires heuristically, and explicit `.react` always wins over a later cued inference). Implementation: `nar/src/dialogue/attribution.ts` (`inferReactionFromUtterance` — conservative cue regexes, priority reject→correct→redirect→clarify→accept, ≤200 chars), `onExchange` binds the cued kind to the immediately preceding turn only (`capture.ts`); `correct` cues pass the utterance as correction text, so formalize/sidecar/digest engage naturally. Cue list is deliberately small — extend only alongside a precision re-run of Bench 76.
...
- All in-plan items and all decision-point gates (DQ2 via Bench 76, DQ6 via Bench 75) are closed. What remains are §12 *consumer-side* extension points (strategy audit → `CognitiveController.adapt()`, retrospective-triggered re-consolidation through `SchemaInductor`) — each needs its own falsifiable bench and touches subsystems outside the flywheel; they belong in a follow-up plan, not here.
- The DQ2 cue list is intentionally minimal. Extending it (e.g., negation handling for `accept`, sarcasm) must re-run Bench 76 with adversarial ground-truth cases before merging.
