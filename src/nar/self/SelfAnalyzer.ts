...
const calcAvg = (values: number[]): number => values.reduce((a, b) => a + b, 0) / (values.length || 1);
  ...
  for (const concept of concepts) {
    ...
    if (isCompound(concept.term)) {
      for (const neighbor of [concept.term.args?.[0], concept.term.args?.[1]].filter(Boolean)) {
        ...
        if (coKey !== termStr) data.coOccurrences.set(coKey, (data.coOccurrences.get(coKey) || 0) + 1);
      }
    }
  }
...
const analyzeTaskPatterns = (nar: NAR | null, metrics: MetricsCollector | null): TaskPatterns =>
  nar && metrics
    ? { avgProcessingTime: metrics.getSummary().throughput?.averageStepDuration ?? 0, queueDepth: 0, dropRate: 0 }
...
export class SelfAnalyzer {
  ...
  async performMetaCognitiveReasoning(): Promise<MetaCognitiveResult> {
    try {
      ...
      return { success: true, patterns, optimizations, timestamp: Date.now(), monitorState: this.monitor.getMonitorState() };
    } catch (error) {
      return {
        success: false,
  ...
  applyOptimizations(): void {
    this.optimizer
      .applyOptimizations({ rulePriorities: [], strategyAdjustments: [], resourceAllocations: [], performanceImprovements: [] })
      .catch(() => {});
  }
  ...
  private async identifyIssues(): Promise<IdentifiedIssues> {
    const issues: IdentifiedIssues = { contradictions: [], inefficiencies: [], resourceIssues: [], performanceIssues: [] };
  ...
  private async applyCorrections(issues: IdentifiedIssues): Promise<CorrectionResult> {
    ...
    for (const issue of issues.resourceIssues || []) {
      if (issue.type === 'high_low_priority_ratio') {
        ...
        if (this.nar.memory) {
          ...
          pendingCorrections.push({ type: 'memory_consolidation', issue: 'high_concept_count', reason: 'consolidation not available' });
        }
      }
    }
    ...
    for (const issue of issues.performanceIssues || []) {
      if (issue.type === 'declining_performance') {
        ...
        this.nar.setConfig({ ...config, maxDerivationsPerStep: Math.max(50, (config.maxDerivationsPerStep || 100) - 20) });
    ...
    for (const issue of issues.inefficiencies || []) {
      if (issue.type === 'slow_inference_chain')
        pendingCorrections.push({ type: 'chain_optimization', issue: 'slow_inference_chain', reason: 'Requires manual review' });
    }
  ...
  private getResourceAnalysis(): Omit<ResourceUsage, 'highPriorityConcepts' | 'lowPriorityConcepts'> {
    ...
    const concepts = this.nar.listConcepts();
    return { conceptCount: concepts.length, avgPriority: calcAvg(concepts.map((c) => c.priority)), memoryUsage: getMemory() };
  }
}
