...
export class NALStdlibLoader {
  ...
  loadModule(name) {
    ...
    // 2. Fallback to Node.js fs if available
      ...
      throw new Error(
        `NAL stdlib module '${name}' not found in virtualFiles and filesystem is unavailable.`
      );
    }
...
export const loadNALStdlib = (interpreter, options) =>
  new NALStdlibLoader(interpreter, options).load();
