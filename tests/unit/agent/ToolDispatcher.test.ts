...
import {dispatchToolCalls} from '../../../agent/src';

describe('ToolDispatcher', () => {
    it('returns artifacts for a successful tool call', async () => {
        const tools = {
            echo: {execute: async (args: Record<string, unknown>) => ({ok: true, echoed: args})},
        };
    ...
    it('surfaces tool-not-found as an error', async () => {
        const result = await dispatchToolCalls([{toolName: 'missing', toolCallId: 'm1', args: {}}], {
            tools: {},
        });
    ...
    it('surfaces tool exceptions as errors', async () => {
        ...
        const result = await dispatchToolCalls([{toolName: 'boom', toolCallId: 'b1', args: {}}], {
            tools,
        });
