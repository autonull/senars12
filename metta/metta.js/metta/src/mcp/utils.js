...
export function withCache(toolFn, options = {}) {
  const { ttlMs = 60000, keyFn = (args) => JSON.stringify(args), cache = new Map() } = options;
...
export function withCircuitBreaker(toolFn, options = {}) {
  const { failureThreshold = 5, successThreshold = 2, timeout = 60000 } = options;
...
export function parallel(...fns) {
  return async (context) => Promise.all(fns.map((fn) => fn(context)));
}
