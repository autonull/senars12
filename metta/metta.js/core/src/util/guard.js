...
function runGuarded(guard, execute, options, isAsync) {
  ...
  const handleGuardError = (err) =>
    errorHandler?.(err, 'guard') ??
    (Logger.error(`[${context}] Guard evaluation failed`, err), defaultValue);
  const handleExecError = (err) =>
    errorHandler?.(err, 'execute') ??
    (Logger.error(`[${context}] Execution failed`, err), defaultValue);

  const guardResult = isAsync
    ? Promise.resolve(guard()).catch(handleGuardError)
    : (() => {
        try {
          return guard();
        } catch (err) {
  ...
  const processResult = (result) => {
    ...
    return isAsync
      ? Promise.resolve(execute()).catch(handleExecError)
      : (() => {
          try {
            return execute();
          } catch (err) {
