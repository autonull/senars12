...
export function logError(error, context = {}, level = 'error', component = '') {
    try {
        ...
        if (typeof error !== 'string' && error) {
            logContext.errorDetails = { name: error.name, message: error.message, stack: error?.stack, code: error.code };
        }
...
function runWithHandling(operation, context, component, options, isAsync) {
    ...
    const runOp = () => isAsync ? Promise.resolve(operation()).then(
        (result) => { if (logOnSuccess) {Logger.info(`[${component}] ${context} completed successfully`);} return result; },
        (error) => { throw error; }
    ) : (() => { const r = operation(); if (logOnSuccess) {Logger.info(`[${component}] ${context} completed successfully`);} return r; })();
...
export function createSafeWrapper(fn, options = {}) {
    ...
    return async (...args) => {
        ...
        catch (error) { logError(error, { context, args: args.length }, 'warn', 'SafeWrapper'); return defaultValue; }
    };
}
