 ...
 *
 * Migrated from: nar/src/cognitive/Observer.ts
 */
...
export class ObserverService {
  ...
  reportConflicts(nar: NAR): string {
    ...
    return (
      `I found ${conflicts.length} conflict(s): ` +
      conflicts
        .slice(0, 3)
        .map((c) => `${c.a} conflicts with ${c.b}`)
        .join('; ') +
