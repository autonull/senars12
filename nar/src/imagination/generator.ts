...
export class ScenarioGenerator {
  ...
  generate(): Scenario {
    const oracle =
      this.config.hiddenModel.length > 0
  ...
  private generateGoalEvent(hiddenRules: HiddenRule[]): Task | null {
    ...
    const term = this.parseTerm(termStr);
    return term
      ? createTask(term, 'goal', Truth.create(priority, 0.8), createBudget(priority))
      : null;
  }
...
export function generateMultipleScenarios(count: number, baseConfig: GeneratorConfig): Scenario[] {
