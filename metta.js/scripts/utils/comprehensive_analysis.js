...
async function analyzeSyllogisticReasoning() {
  ...
  console.log(
    `     Concepts: ${stats1.memoryStats?.memoryUsage?.concepts || stats1.memoryStats?.totalConcepts || 0}`
  );
  console.log(
    `     Total tasks: ${stats1.memoryStats?.memoryUsage?.totalTasks || stats1.memoryStats?.totalTasks || 0}`
  );
  ...
  console.log(
    `     Concepts: ${stats2.memoryStats?.memoryUsage?.concepts || stats2.memoryStats?.totalConcepts || 0}`
  );
  console.log(
    `     Total tasks: ${stats2.memoryStats?.memoryUsage?.totalTasks || stats2.memoryStats?.totalTasks || 0}`
  );
  ...
  for (let cycle = 1; cycle <= 5; cycle++) {
    ...
    console.log(
      `    Before step - Beliefs: ${beforeBeliefs.length}, Tasks: ${beforeStats.memoryStats?.memoryUsage?.totalTasks || beforeStats.memoryStats?.totalTasks || 0}`
    );
    ...
    console.log(
      `    After step  - Beliefs: ${afterBeliefs.length}, Tasks: ${afterStats.memoryStats?.memoryUsage?.totalTasks || afterStats.memoryStats?.totalTasks || 0}`
    );
    ...
    if (hasAC) {
      ...
      if (acBelief) {
        console.log(
          `       Derived: ${acBelief.term?.toString?.() || acBelief.term} with ${acBelief.truth.frequency},${acBelief.truth.confidence}`
        );
      }
      break;
    }
