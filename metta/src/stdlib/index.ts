...
import { sym, expr, num, str, isExpression } from '../types/ast.js';
import type { MeTTaAtom, NumberAtom, ExpressionAtom } from '../types/ast.js';
...
const mapOp = defineOp('map', (fn: MeTTaAtom, list: MeTTaAtom) => {
...
const filterOp = defineOp('filter', (pred: MeTTaAtom, list: MeTTaAtom) => {
  if (!isConsAtom(list)) return list;
...
const ifOp = defineOp('if', (cond: MeTTaAtom, thenBranch: MeTTaAtom, elseBranch: MeTTaAtom) => 
  cond.kind === 0 && cond.value === 'True' ? thenBranch : elseBranch, { pure: false });

const letOp = defineOp('let', (name: MeTTaAtom, value: MeTTaAtom, body: MeTTaAtom) => body, { pure: false });
const matchOp = defineOp('match', (pattern: MeTTaAtom, target: MeTTaAtom) => sym('True'), { pure: false });

const printOp = defineOp('print', (msg: MeTTaAtom) => {
  console.log(msg);
  return sym('True');
}, { pure: false });

const errorOp = defineOp('error', (msg: MeTTaAtom) => {
