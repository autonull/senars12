...
import { GovernancePolicyEngine, PatchRiskClassifier } from '../../nar/src/governance/pipeline.js';
...
const base = {
  ...
  affectedComponents: ['tools'] as Array<'tools'>,
...
describe('todo7: governance pipeline', () => {
  it('guardrail touch → HIGH; clean patch → LOW', () => {
    ...
    const high = c.classify({
      ...base,
      proposalId: uuidv4(),
      affectedFiles: ['nar/src/kernel/KernelRewardGate.ts'],
    });
    expect(high.risk).toBe('HIGH');
    expect(high.score).toBeGreaterThanOrEqual(50);
    const low = c.classify({
      ...base,
      proposalId: uuidv4(),
      affectedFiles: ['nar/src/tools/sleep.ts'],
    });
  ...
  it('critical component → HIGH', () => {
    ...
    const high = c.classify({
      ...
      proposalId: uuidv4(),
      affectedFiles: ['x.ts'],
      affectedComponents: ['approval-logic'] as Array<'approval-logic'>,
    });
    expect(high.risk).toBe('HIGH');
  ...
  it('large churn and coverage drop raise score', () => {
    ...
    const r = c.classify({
      ...base,
      proposalId: uuidv4(),
      affectedFiles: ['nar/src/tools/x.ts'],
      linesAdded: 400,
      linesRemoved: 200,
      coverageDelta: -8,
    });
  ...
  it('policy: HIGH always human; LOW auto-merges only in low-risk-auto-merge', () => {
    ...
    expect(e.decide({ risk: 'HIGH', score: 90, factors: [] }, 'low-risk-auto-merge').action).toBe(
      'REQUIRE_HUMAN_REVIEW'
    );
    expect(e.decide({ risk: 'LOW', score: 0, factors: [] }, 'sandbox-execute').action).toBe(
      'REQUIRE_HUMAN_REVIEW'
    );
    expect(e.decide({ risk: 'LOW', score: 0, factors: [] }, 'low-risk-auto-merge').action).toBe(
      'AUTO_MERGE'
    );
    expect(
      e.decide({ risk: 'MEDIUM', score: 25, factors: [] }, 'low-risk-auto-merge')
    ).toMatchObject({ action: 'CREATE_PR', reviewers: 1 });
    expect(
      e.decide({ risk: 'LOW', score: 0, factors: [] }, 'human-approved-production').action
  ...
  it('record emits audit event', () => {
    ...
    const ev = e.record(
      proposal,
      { risk: 'LOW', score: 0, factors: [] },
      { action: 'AUTO_MERGE', reason: 'ok' },
      'low-risk-auto-merge'
    );
