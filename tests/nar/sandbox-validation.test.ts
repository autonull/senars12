...
describe('todo7: sandbox validation', () => {
  it('validator approves in-range known knobs; rejects unknown/out-of-range/non-knob', () => {
    ...
    expect(
      v.validate(
        new SelfRewardGate().propose(
          'knob-tune',
          { knob: 'rankingMaxAdmissions', value: 200 },
    ...
    expect(
      v.validate(
        new SelfRewardGate().propose(
          'knob-tune',
          { knob: 'rankingMaxAdmissions', value: 5000 },
          'self-config-proposal'
        )
      )
    ).toMatchObject({ approved: false });
    expect(
      v.validate(
        new SelfRewardGate().propose('schema-promotion', { schema: 's' }, 'self-config-proposal')
      )
    ).toMatchObject({ approved: false });
  });
  it('validated knob-tune applies via actuator; unvalidated stays queued', () => {
    ...
    const actuators = {
      applyKnob: (k: string, v: number) => {
        applied.push([k, v]);
      },
    };
    const ok = router.route(
      gate.propose(
        'knob-tune',
        { knob: 'rankingMaxAdmissions', value: 200 },
        'self-config-proposal'
    ...
    const noValidator = router.route(
      gate.propose('knob-tune', { knob: 'maxLoops', value: 3 }, 'self-config-proposal'),
      'sandbox-execute',
      actuators
    );
    expect(noValidator.route).toBe('sandbox-validate');
    const locked = router.route(
      gate.propose('knob-tune', { knob: 'maxLoops', value: 3 }, 'self-config-proposal'),
      'observe-only',
      actuators,
      validator
