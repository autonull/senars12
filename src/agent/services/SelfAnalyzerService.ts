 ...
 *
 * Migrated from: src/nar/self/SelfAnalyzer.ts
 */

import type {NAR} from '../../nar/nar.js';
import type {MetricsCollector} from '../../nar/metrics';
import type {Concept} from '../../nar/memory';
import {isCompound} from '../../nar/terms';
import type {Optimizations} from '../../nar/self/SelfOptimizer';
import {SelfOptimizer} from '../../nar/self/SelfOptimizer';
...
const analyzeTaskPatterns = (
  ...
  nar && metrics
    ? {
        avgProcessingTime:
          metrics.getSummary().throughput?.averageStepDuration ?? 0,
        queueDepth: 0,
        dropRate: 0,
      }
...
export class SelfAnalyzerService {
  ...
  private async applyCorrections(issues: IdentifiedIssues): Promise<CorrectionResult> {
    ...
    for (const issue of issues.performanceIssues || []) {
      if (issue.type === 'declining_performance') {
        ...
        this.nar.setConfig({
          ...config,
          maxDerivationsPerStep: Math.max(
            50,
            (config.maxDerivationsPerStep || 100) - 20
          ),
        });
        appliedCorrections.push({
  ...
  > {
    if (!this.nar)
      return { conceptCount: 0, avgConceptPriority: 0, memoryUsage: getMemory() };
    const concepts = this.nar.listConcepts();
    return {
      conceptCount: concepts.length,
  ...
  async getCapabilitySnapshot(): Promise<CapabilitySnapshot> {
    ...
    return {
      ...
      lmProviders: this.nar.getLMClient
        ? [this.nar.getLMClient()?.provider || 'none']
        : [],
      pipelineStages: [
        'InputNormalizer',
        'AuthChecker',
