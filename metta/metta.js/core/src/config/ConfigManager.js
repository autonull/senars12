...
export class ConfigManager {
  ...
  constructor(
    defaults = {},
    { schema = null, autoValidate = false, freeze = false, recordHistory = false } = {}
  ) {
  ...
  set(path, value, { validate = true, persist = false, override: overrideName = null } = {}) {
    if (this.#frozen) {
      throw new ConfigurationError('Config is frozen', { key: path });
    }
    const validator = this.#validators.get(path);
    if (validate && validator && !validator(value)) {
      throw new ConfigurationError(`Invalid value for ${path}`, { key: path, value });
    }

    if (overrideName) {
      if (!this.#overrides.has(overrideName)) {
      ...
      const target = keys.slice(0, -1).reduce((cur, key) => {
        cur[key] ??= {};
        return cur[key];
      }, this.#config);
    ...
    if (persist || this.#recordHistory) {
      this.#history.push({ key: path, value, timestamp: Date.now() });
    }
    if (validate && this.#autoValidate) {
      this.#validate();
    }
  ...
  define(key, defaultValue, validator = null, description = '') {
    if (this.#frozen) {
      throw new ConfigurationError(`Cannot define config after freeze: ${key}`);
    }
    if (validator && !validator(defaultValue)) {
      throw new ConfigurationError(`Invalid default value for ${key}`, { key });
    }
  ...
  update(updates, { deep = true, validate = true } = {}) {
    if (this.#frozen) {
      throw new ConfigurationError('Config is frozen');
    }
  ...
  reset(path = null) {
    if (path) {
      const entry = this.#entries.get(path);
      if (entry) {
        entry.value = this.#defaults[path] ?? entry.value;
  ...
  #validate() {
    ...
    try {
      return this.#schema(this.#config);
    } catch (error) {
  ...
  getMeta(key) {
    ...
    return {
      ...
      listenerCount: this.#keyListeners.get(key)?.length ?? 0,
    };
  }
  ...
  getStats() {
    ...
    return {
      ...
      listenerCount: this.#keyListeners.size,
    };
  }
  ...
  clone() {
    const cloned = new ConfigManager(this.#defaults, {
      schema: this.#schema,
