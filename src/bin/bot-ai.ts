...
async function main(): Promise<void> {
  ...
  const { agent, sessionManager, episodicMemory, profile, appConfig } = await createAgentFromEnv();
  ...
  // Config-file `irc` block fills values env doesn't set (env wins over config).
  const ircCfg = readIRCConfig(appConfig.irc);
  const ircConfig = configs.find((c) => c.type === 'irc')?.config as
    | Record<string, unknown>
    | undefined;
  if (ircConfig) {
    ircConfig.server = ircCfg.server;
    ircConfig.port = ircCfg.port;
    ircConfig.nick = ircCfg.nick;
    ircConfig.channels = ircCfg.channels;
  }
  logger.info(
