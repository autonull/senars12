...
- **Still open (deliberately):**
  - F3 real-provider legs (`LM_PROVIDER=ollama`/`llamacpp` on/off token-reduction deltas) — needs a model-cached machine (no ollama daemon and no cached encoder on the 2026-09-20 box; transformers.js would download on first use).
