...
export class MettaCommandParser {
  ...
  #mergeSendContinuations(lines: string[]): string[] {
    ...
    while (i < lines.length) {
      const line = lines.at(i) as string;
      ...
      while (i < lines.length && !this.#isKnownCommand(lines.at(i) as string)) {
        let cont = (lines.at(i) as string).trim();
  ...
  #parseLine(line: string): ParsedCommand | null {
    ...
    if (!cmd || !LLM_COMMANDS.includes(cmd as LlmCommand)) return null;

    const llmCmd = cmd as LlmCommand;

    if (llmCmd === 'write_file' || llmCmd === 'append_file') {
      let filename = '';
      let content = '';
      if (rest.startsWith('"')) {
