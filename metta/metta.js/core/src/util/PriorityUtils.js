...
export class PriorityCalculator {
  ...
  static calculate(factors = {}, weights = {}, options = {}) {
    ...
    const { normalize = true, minPriority = 0 } = options;
  ...
  static calculateInputPriority(inputFactors = {}, config = {}) {
    ...
    const { truthValue = null, taskType = 'BELIEF', complexity = 1 } = inputFactors;
...
export const PriorityStrategies = {
  ...
  concept: (concept, weights = {}) => {
    if (!concept) {
      throw new RuntimeError('Concept is required for priority calculation', 'concept', {
        weights,
      });
    }
  ...
  input: (inputData, config = {}) => {
    if (!inputData) {
      throw new RuntimeError('Input data is required for priority calculation', 'input', {
        config,
      });
    }
...
export class PriorityScorer {
  ...
  score(entity, strategy = 'dynamic', options = {}) {
    ...
    if (!strategyFn) {
      throw new RuntimeError(`Unknown priority scoring strategy: ${strategy}`, 'score', {
        strategy,
    ...
    try {
      ...
      throw new RuntimeError(`Error in priority scoring: ${error.message}`, 'score', {
        strategy,
  ...
  batchScore(entities, strategy = 'dynamic', options = {}) {
    if (!Array.isArray(entities)) {
      throw new RuntimeError('Entities must be an array', 'batchScore', { entities, strategy });
    }
