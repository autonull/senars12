...
import {
  ...
  KernelRewardGate,
  SelfRewardGate,
} from '../../nar/src/kernel/KernelRewardGate.js';
...
describe('todo7: reward domain split', () => {
  ...
  it('ExternalRewardGate ingests; SelfRewardGate builds risk-tiered proposals', () => {
    const ext = new ExternalRewardGate().ingest({
      rewardSignal: 0.8,
      rewardType: 'extrinsic',
      targetId: 'focus-1',
    });
...
describe('todo7: autonomy state machine', () => {
  it('enforces legal transitions and approval authority', () => {
    ...
    expect(
      gate.requestModeChange('human-approved-production', 'external-governance', 'c2').changed
    ).toBe(true);
