...
export class ToolRegistry extends BaseComponent {
  ...
  autoRegisterFromModule(toolModule, options = {}) {
    ...
    const toolsToRegister = discoveredTools.filter((tool) => {
      ...
      if (include && !include.includes(toolId)) {
        return false;
      }
      return true;
    });
  ...
  #matchesCriteria(metadata, criteria) {
    ...
    if (
      criteria.supportsStreaming !== undefined &&
  ...
  getStats() {
    return {
      ...
      lastDiscovery:
        Math.max(...Array.from(this.discoveredTools.values()).map((d) => d.timestamp ?? 0)) ?? 0,
    };
  }
}
