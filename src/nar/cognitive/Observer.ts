...
export type CognitiveState = 'normal' | 'confused' | 'bored' | 'overloaded';
export type CognitiveAction = 'continue' | 'resolve-conflicts' | 'explore' | 'consolidate';
...
export class Observer {
    ...
    private resolveConflicts(nar: NAR): void {
        ...
        for (const conflict of conflicts.slice(0, 5)) {
            const term = atom(conflict.a.split('-->')[0]?.replace('(', '').trim() ?? 'unknown');
    ...
    private exploreMemory(nar: NAR): void {
        ...
        for (const concept of underconnected) {
            ...
            const related = concepts.filter(c =>
                c !== concept && this.termOverlap(termStr, c.term.toString()) > 0,
            ).slice(0, 3);
    ...
    private findConflicts(beliefs: Array<{ term: { toString(): string }; truth?: { f: number; c: number } }>): Array<{ a: string; b: string }> {
        const termMap = new Map<string, Array<{ term: string; f: number }>>();
        for (const b of beliefs) {
            if (!b.truth) continue;
            const key = this.extractSubject(b.term.toString());
            const list = termMap.get(key) ?? [];
            list.push({ term: b.term.toString(), f: b.truth.f });
            termMap.set(key, list);
        }
    ...
    private extractSubject(termStr: string): string {
        const match = termStr.match(/\(([^ ]+)\s+-->/);
        return match?.[1] ? match[1].replace('(', '') : termStr;
    }
