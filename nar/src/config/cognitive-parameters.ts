...
export function validateParameters(params: Partial<CognitiveParameters>): {
  ...
  if (params.priority) {
    ...
    if (p.initialPriority < 0 || p.initialPriority > 1)
      errors.push('priority.initialPriority must be in [0, 1]');
    if (p.directMentionBoost < 0 || p.directMentionBoost > 1)
      errors.push('priority.directMentionBoost must be in [0, 1]');
  }
