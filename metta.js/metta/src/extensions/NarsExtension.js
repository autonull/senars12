...
export class NarsExtension {
  ...
  _registerStubs() {
    ...
    [
      ...
      'nar-deserialize',
      'nar-snapshot',
      'nar-snapshot-compare',
      'nar-stamps',
      'nar-recent-derivations',
      'nar-focus-sets',
      'nar-focus-create',
      'nar-focus-switch',
      'nar-revision',
  ...
  _registerGoalOps() {
    const findGoal = (term) =>
      this.nar.taskManager.findTasksByType('GOAL').find((g) => g.term?.toString?.().includes(term));
    ...
    this.ground.add('nar-goal-status', (termAtom) => {
      ...
      return Term.grounded(
        `(goal-status :${match.budget?.priority >= 0.5 ? 'active' : 'pending'} "${term}" :priority ${match.budget?.priority?.toFixed(2) ?? '?'})`
      );
    });
  }
  ...
  _registerStateOps() {
    ...
    this.ground.add('nar-snapshot', (labelAtom) => {
      ...
      try {
        const data = {
          ...
          beliefs: this.nar
            .getBeliefs()
            .map((b) => ({ term: b.term?.toString?.(), truth: b.truth })),
          goals: this.nar
            .getGoals()
            .map((g) => ({ term: g.term?.toString?.(), priority: g.budget?.priority })),
        };
    ...
    this.ground.add('nar-snapshot-compare', (labelA, labelB, thresholdAtom) => {
      const threshold =
        Number.parseFloat(thresholdAtom?.value ?? thresholdAtom?.name ?? '0.1') || 0.1;
      try {
        const dir = resolve(process.cwd(), 'memory/snapshots');
  ...
  _registerFocusOps() {
    this.ground.add('nar-focus-sets', () => {
      ...
      const sets = Object.entries(stats.focusSets || {}).map(
        ([name, s]) => `${name}: ${s.size ?? '?'} tasks`
      );
  ...
  _registerDerivationOps() {
    ...
    this.ground.add('nar-recent-derivations', (countAtom) => {
      const count = Number.parseInt(countAtom?.value ?? countAtom?.name ?? '5') || 5;
    ...
    this.ground.add('nar-stats', () => {
      ...
      const parts = [
        ...
        stats.taskManagerStats
          ? `tasks: ${stats.taskManagerStats.totalTasksCreated ?? '?'} created`
          : null,
