...
- [x] D4 remote manifold client/server over `/v1/systemone` shape (done 2026-09-20 — see note)
- [x] Bench 21 + 22 + 24

**Progress Notes (2026-09-20, Phase D complete except D5):**
...
- **D4 (done 2026-09-20):** `nar/src/lm/system-one/http-manifold.ts` — `createHttpManifold({endpoint, embeddingCache, timeoutMs?, fetchImpl?})` implements `JudgmentManifold`: reads the context embedding from the **shared local cache** (`cache.read(pointer)` — pointers are cache-scoped), POSTs `{contextEmbedding, queries, trusted:false}` to `<endpoint>/v1/systemone`, zod-validates propositions (discriminated classify/evaluate schema; malformed ⇒ throw) and flips `health.ready/breakerOpen` on failure (fail-closed; dispatcher Tier-3 degrades, no silent pass-through). `consensus` = one batched pass, agreement 1 (endpoint owns its internal fan-out). Server: `scripts/system-one-server.ts` (node:http over `handleSystemOneRequest`; ** IncomingMessage has no `.json()` — the transport must buffer the body and pass `{json: async () => parsed}` — that stub contract is what `handleSystemOneRequest` accepts). NAR wiring: `systemOne.manifold.provider: 'http'` + `endpoint` (+ additive `manifold.timeoutMs`) ⇒ `createHttpManifold` replaces the local Tier-1; local encoder still produces context embeddings; untrusted re-entry seeding stays the existing `admitRemotePropositions`/gate source-quality path (`LLM_PRIOR` ceiling 0.5 asserted in tests). Test: `tests/nar/todo16c-remote-manifold.test.ts` (3: round-trip + ceiling, consensus, malformed-response + dead-endpoint fail-closed). Real encoder server demo: `pnpm exec tsx scripts/system-one-server.ts`.
- **D5 (done 2026-09-20):** WASI encoder-head bundle shipped — no toolchain choice needed. `nar/src/lm/system-one/wasi-head-bundle.ts`: `emitHeadBundleWasm` emits the `.wasm` binary byte-by-byte in TS (type/func/memory/export/code/data sections; unrolled clamp01(w·z+b) chain with z-score mean/std baked into the data segment; zero imports — deny-by-default sandbox). `writeHeadBundle` writes `head.wasm` + `config.json` + `MODEL_DIGEST`; `loadHeadBundle` SHA256-verifies (DigestMismatchError fail-closed), requires `assertWasmPathContained` allowedPaths + zero-import module, timeout-guards eval. **Gotchas for future wasm work:** `i32.const` needs *signed* LEB128 (unsigned encoding turns 64..127 into negatives — the cause of "memory access out of bounds" at addresses ≥ 64); `f32.load` opcode is 0x2A (0x2C is i32.load8_s); f32 ops: add 0x92, sub 0x93, mul 0x94, min 0x96, max 0x97; function bodies start with the local-decl vector (0x00 for none); `WebAssembly.instantiate(module, imports)` returns the Instance directly (only the bytes overload returns `{module, instance}`). `@wasmer/wasi` is broken in Node 26 ESM (`createWasmModuleSandbox` can't resolve it) — that's why the zero-import + `withTimeout` posture was used; the 'wasi' provider path in `HeadRuntimeConfig` still routes through the broken `createWasiSandbox` and remains unwired. `scripts/system-one-train.ts --wasi` emits the bundle for a trained linear head. Bench 24 extended (`todo16c-train.test.ts` D5 case: sandbox-loaded eval matches the trained model within 1e-5; digest mismatch fails closed).
...
- Next per the build sequence: **Phase I (usability)** — done. D5 (WASI bundle) carries over from Phase D; F3/F4 remain.
- Phase D deferred item: D5 WASI head bundle (~0.5–1d, toolchain choice pending); no pending bench (Bench 21/22/24 green, D4 done 2026-09-20).
...
- [x] E3 `judgeCascade`
- [x] E4 agent-trace grading → dataset labels (2026-09-20; see progress note)
- [x] E5 (optional) wake gate — done 2026-09-20 (see note)
- [x] Bench 23

**E5 (done 2026-09-20):** `nar/src/lm/system-one/wake-gate.ts` (exported from the subpath index) — `createWakeGate({manifold, embeddingCache, budget?, wakeThreshold=0.7, notYetThreshold=0.4, dataset?, source?})`. One `relevance` evaluate per sleep note: score ≥ wake ⇒ `wake`; ≥ notYet ⇒ `not_yet`; else `unrelated`. **User input always wakes without a judgment.** Fail-open on abstain / `calibration.fitted !== true` / manifold failure / missing proposition ⇒ `wake` (timers waking is the pre-gate behavior; uncertainty must never stall a sleeping agent). Dataset labels recorded hash-only when supplied. There is no live sleep/wake loop in the codebase yet — the gate is a library utility awaiting a consolidation-hook consumer; wire it where a wake decision point appears. Test: `tests/nar/todo16c-wake-gate.test.ts` (5: user-input short-circuit, band mapping, abstain/unfitted fail-open, manifold failure fail-open, dataset label).

**Progress Notes (2026-09-20, Phase E core complete — E5 optional remains):**
...
- **E4 (done 2026-09-20):** `nar/src/lm/system-one/trace-grader.ts` — `createTraceGrader({manifold, embeddingCache, dataset?, budget?, source?})` grades the completed cycle: narration → `groundedness` (evaluate-with-rubric convention, same as `ManifoldRLAgent`; classify queries would route to the `task_type` head — manifold.ts:156), each executed tool call → `risk` with its own `cache.write('<command> <ok|error>')` embedding. Grades are hash-only `DistillationLabel`s; `observed = success ? 0 : 1` for risk (ground truth), none for groundedness. Wiring: `NAR.initializeSystemOne` creates the grader + a `JudgmentDataset` when `systemOne.distillation.datasetPath` is set (auto-flush started — D3's first production `startAutoFlush` call site; the default-schema path means an enabled System One writes to `./data/...`); `getSystemOneTraceGrader()` accessor; `nar/src/agent/index.ts` passes it to `Agent`; `CycleHost.traceGrader` invoked at cycle end (after tool results, best-effort — grading never blocks; skipped when narration is empty). Tests: `tests/nar/todo16c-trace-grading.test.ts` (3) + `tests/unit/core/agent.test.ts` E4 wiring case. **Manifold + grader must share ONE `EmbeddingCache` — pointers are cache-scoped (G4 invariant; two caches = X6 regression).** PreferenceCollector hook-up intentionally deferred: `addPreference` needs two full trajectories per comparison; trace-level grading produces one element set per cycle — a trajectory-store/pairing design decision is needed first (recorded below).
...
- [x] F3 token-reduction + fan-out latency report (mock leg done 2026-09-20 — see note; real-provider legs need a model-cached machine)
...
- **F3:** mock leg done 2026-09-20 — `scripts/system-one-bench.ts` (`pnpm bench:system-one`, `LM_PROVIDER=mock`): on/off P50/P99 input latency, LM calls/spend, Tier-0 parse-failure and judgment-event deltas over a 10-utterance × 3-cycle workload → `.reports/system-one-onoff.{json,md}`; wired into the F8 `systemone-benches` CI job. **Mock-leg findings:** ON adds ~0.1 ms P50 / ~3.4 ms P99 input overhead (gate path) with zero LM calls (cortex off); the OFF leg rejects all NL utterances at Tier-0 parse (0 vs 30 failures) — the ingress acceptance delta. **Real-provider legs** (token-reduction delta, cortex-mediated cycles) still need a model-cached machine — run `LM_PROVIDER=ollama pnpm bench:system-one` where ollama/llamacpp is cached.
- **F4:** resolved (2026-09-20) — terminal-attribution fix shipped; TD(λ) sweep measured as a negative result (see §10.6 F4 residual closure). Remaining smoke-level gap is tie-break RNG noise + exploration schedule, not credit assignment.
...
- ~~D4 remote manifold~~ (done 2026-09-20, see Phase D notes).
...
**F4 residual closure (2026-09-20, follow-up session):**
...
**F3 real-provider legs done (2026-09-20) — llamacpp-embedded (no ollama involvement):**
- GGUF models were already cached (`LM_LLAMACPP_MODEL=.models/Qwen3.5-0.8B-Q4_0.gguf`, `LM_PROVIDER=llamacpp-embedded`); the bench script no longer hardcodes `LM_PROVIDER=mock` (the package script default inside the script is `'mock'`, so CI's offline leg is unchanged) and real legs enable `systemOne.cortex.provider` so LM calls flow. Per-provider report files (`system-one-onoff[-<provider>].{json,md}`) keep the CI mock report intact. `llamacpp-embedded` added to the `systemOne.cortex.provider` zod enum.
- **Real defect found and fixed by the real leg:** the `narsese-term` GBNF *name* was passed to node-llama-cpp as if it were grammar text (`runWithGrammar('narsese-term', …)` → `createGrammar({grammar: 'narsese-term'})` → "error parsing grammar: expecting ::=") — every grammar-constrained generation failed silently to the stub path. Fix: `LMService.runInGrammarScope` resolves named grammars (`narsese-term`, `single-word`) to their shipped GBNF text via `loadGrammar` before entering the scope; raw GBNF passes through. This un-breaks Bench 16's GBNF segment for real providers.
- **Real-leg measurements** (Qwen3.5-0.8B Q4, 10 NL utterances ×3 cycles): ingress ON adds ~0.08 ms P50 / ~3.4 ms P99 with **0 LM calls** (heads are local) and +57 judgment events; OFF rejects all 30 NL inputs at Tier-0 parse. Translation workload through the real `lm-narsese-translation` rule: ON (cortex candidate synthesis + manifold select) 11 calls / 9300 tokens / **28 tasks admitted**; OFF (generative LM path) 10 calls / 3200 tokens / 4 tasks admitted — candidate synthesis costs more raw tokens but admits 7× the tasks; **tokens per admitted task: 332 (ON) vs 800 (OFF)**, a 2.4× cost-per-useful-output win for the System One pipeline.
- **Still open (deliberately):**

**Second follow-up pass (2026-09-20):**
- **E4 follow-up (b) done:** `TraceGradeInput.egress?: {grounded, score?}` — `runCycleStream` captures the egress verdict (and `narrate()` now returns `{text, egress}`) and passes it to the cycle-end grader call; `createTraceGrader` records groundedness with `observed = grounded ? 1 : 0` when a verdict is present (absent verdict still records no `observed`). CycleHost/Agent traceGrader trace shapes widened additively.
- **E4 follow-up (a) done:** `nar/src/rlfp/trajectory-store.ts` — `TrajectoryStore`: per-cycle `recordCycle({correlationId, timestamp, steps, grades})` (append-only JSONL when `path` given, malformed-line-tolerant `load()`), `pairForPreference()` pairs the two most recent cycles (grade order: groundedness desc, then mean risk asc; equal/abstained grades ⇒ `SKIP`). Wired in `createAgent`: each graded cycle is recorded and a non-SKIP pair feeds `nar.rlfp.addPreference(preferredNarration, rejectedNarration)`. Path from new optional `systemOne.distillation.trajectoryPath` (schema) → `lifecycle.ts` → `CreateAgentConfig.trajectoryStorePath`.
- **D3 compaction done:** `JudgmentDataset.compact(datasetPath, sidecarPath?)` — dedupes JSONL rows by evidenceId (last wins), counts malformed rows as dropped, prunes sidecar `<evidenceId>.f32` files with no live row; `scripts/system-one-compact-dataset.ts` CLI (defaults from config).
- Tests: `todo16c-trace-grading` extended (egress observed 0/1/undefined), new `tests/nar/todo16c-trajectory-store.test.ts` (5: pairing semantics, risk tie-break, JSONL persistence, malformed tolerance, compaction).
- **Remaining after this pass:** F3 real-provider legs (above); H1 batched `doEmbed` (revisit only if Bench 2 latency demands); `plausibility`/`truthProbability` head deliberately not in `HEAD_SPECS` (rationale above).
